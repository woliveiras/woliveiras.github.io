...
The manual scenario runner has a `prompt-cache-fallback` scenario that warms the cache once, simulates a Langfuse prompt fetch timeout, and runs the graph again:
