...
The Langfuse UI usually shows `LANGFUSE_BASE_URL="http://localhost:3000"` for local SDKs running directly on the host. The Collector runs inside Docker, so `localhost` would point to the Collector container itself. `host.docker.internal` points from the Collector container back to Langfuse exposed on the host.
...
The logs should show the Collector starting without exporter configuration errors. If `LANGFUSE_SECRET_KEY`, `LANGFUSE_PUBLIC_KEY`, or `LANGFUSE_BASE_URL` is missing, fix `.env` before running the smoke test.
...
## What should exist before Chapter 14
...
4. `.env` containing `LANGFUSE_SECRET_KEY`, `LANGFUSE_PUBLIC_KEY`, and `LANGFUSE_BASE_URL`.
