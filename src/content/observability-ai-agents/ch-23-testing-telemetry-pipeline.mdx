...
Open `tests/test_privacy.py` and add this test:
...
This is a small test, but it protects an important rule from Chapters 8, 14, 15, 17, and 26: content is not exported by default. It also keeps the test independent of Langfuse and the Collector.
...
def test_user_feedback_score_is_bounded(monkeypatch) -> None:
    fake = FakeLangfuse()
    monkeypatch.setattr(scores, "langfuse", fake)
...
Now the test suite fails when telemetry parentage, privacy defaults, Langfuse context, or score contracts drift. This kind of regression test is the only way to protect the telemetry pipeline from accidental breakage.
