...
Unit tests can call producer and consumer handlers in one process while still exercising real `inject` and `extract` operations. Collector integration tests are a separate layer: they verify that serialization, processors, sampling, and backend export preserve the relationships already validated in memory. Chapter 23 builds both test layers.
