...
def redact_text(value: str) -> str:
    ...
    if "ORDER-" in redacted:
        redacted = redacted.replace("ORDER-", "ORDER-[redacted]-")
    return redacted
...
APPROVAL_DIGEST_KEY = b"demo-only-replace-with-secret-key"
...
```sh
python -m agent_observability.manual_scenarios feedback
```
...
Open or create (if you created it before on chapter 12 or 16) `src/agent_observability/signals.py`. The file defines a bounded negative signal event and a helper to record it in telemetry:
...
```sh
python -m agent_observability.manual_scenarios negative-signal
```
