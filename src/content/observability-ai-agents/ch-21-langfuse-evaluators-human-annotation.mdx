...
python -m agent_observability.score_scenarios feedback --trace-id <langfuse-trace-id>
python -m agent_observability.score_scenarios policy --trace-id <langfuse-trace-id>
python -m agent_observability.score_scenarios correctness --trace-id <langfuse-trace-id> --correct
python -m agent_observability.score_scenarios session \
...
For the evaluator we created above, this is acceptable judge input:

```text
Question: Where is my order ORDER-123?
