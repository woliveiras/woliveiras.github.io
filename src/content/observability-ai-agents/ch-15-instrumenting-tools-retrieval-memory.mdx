...
If you copied the complete `fetch_order_status` wrapper from the previous section, this validation code is already included. Use this section to understand and verify the specific changes that make validation observable.

In this exercise, update `fetch_order_status` so it records four controls separately: argument validation, authorization, approval requirement, and execution outcome. This makes incidents easier to classify: invalid arguments, authorization denial, missing approval, and execution failure are different problems.

Add this helper to `src/agent_observability/tools.py`, above `fetch_order_status`:
...
Then update the beginning of `fetch_order_status`, inside the span and before `authorize_order_read`:
...
Finally, set the execution result after the service call succeeds:
