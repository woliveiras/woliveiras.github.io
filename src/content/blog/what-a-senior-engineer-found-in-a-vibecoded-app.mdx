...
Half the industry seems to be arguing about whether vibecoding is dangerous. My take is different: it is genuinely impressive, and the people doing it deserve credit for what they ship. The conversation usually stops there though, and that is where it gets interesting.

In April 2026 I joined a platform that had been built by someone with deep expertise in Design and zero formal background in software development, using Cursor as their primary tool. In roughly two months they shipped a full multi-tenant healthcare platform running in production with real users: four user roles, real-time chat, an installable PWA, Google OAuth, tokenized invite flows, OpenAI-powered report generation, and clinic management. Building that in two months is not trivial, and I do not want to pretend otherwise.

What follows is what I found when I read the code, what I did about it, and why pairing a senior engineer's judgment with AI assistance got us further, faster, than either would have alone.

## What was built before I arrived

The platform is built for pediatric therapy clinics. It connects parents, therapists, supervisors, and school staff around a child's development: scheduling, clinical reports, progress tracking, multi-party chat, LGPD-sensitive data. In a system like this a security failure is not just a technical problem, it is a real risk to children's privacy and safety.
...
The point of this section is not "I fixed a lot of things." It is what kind of judgment each fix required, and where AI fit into it. I changed a lot of code in those four weeks, but I am focusing here on the critical issues and the structural patterns I changed, not the incidental ones.
...
"I used AI" can mean almost anything, so let me be specific. The split was always the same: I diagnosed, AI executed.

The RLS recursion was not something I asked AI to find, I found it by reading the schema. What AI did was help me write the corrected policies and generate the tests around them. I knew what was wrong and what the fix should look like; AI made producing the code faster.
...
It would be easy to read this post as a critique of vibecoding, but that is not where I land. The person who built the platform made decisions that made my work significantly easier, even when those decisions created problems I had to fix.
...
The takeaway is not "vibecoding is dangerous" or "you need a senior engineer to validate AI output." The honest version is narrower than either of those.

What worked here was a sequence: a non-developer using AI to build something genuinely useful and complex at impressive speed, then an experienced engineer using AI to fix, harden, and extend it at an equally impressive speed. Neither approach would have produced the same result on its own. The vibecoded version was real, in production, used by real people. Without the engineering work it would have stayed there with critical vulnerabilities and the kind of debt that compounds. Without the vibecoded foundation, the engineering work would have started from scratch, and those users would have waited a lot longer.
