...
Working on a map application used to find products in a store, which uses a classic routing algorithm, I started wondering how we could improve our solution to be smarter and more efficient. Nothing seemed better for this task than looking at how the big players work and understanding the technologies that make them so effective. Google Maps, Waze, and CityMapper are examples of applications that I use daily and that make me curious about how they work under the hood.

While researching these applications, I found that they leverage a combination of classical algorithms and modern machine learning techniques to provide fast and accurate routing services. This is particularly interesting because routing algorithms have evolved significantly over the years, adapting to the increasing complexity of road networks and traffic patterns. From `Dijkstra's` and `A*` algorithms that laid the foundation for pathfinding, to `Contraction Hierarchies` that enable real-time routing on continental scales, and finally to `Graph Neural Networks` that predict travel times by learning from vast amounts of traffic data.
...
In routing algorithms, the priority queue simply orders frontier nodes by their current best-known distance from the source: the node with minimal tentative distance is expanded next, and its neighbors are relaxed and reinserted if a shorter path is discovered.
...
The node ordering heuristic proves critical to performance. A simple but effective priority function combines multiple factors: edge difference (shortcuts added minus edges removed, preferring nodes that reduce total edges), deleted neighbors (count of already-contracted neighbors, ensuring uniform progression), and shortcut cover (how many queries would benefit from these shortcuts). **Experiments reported na literatura de Contraction Hierarchies mostram que, para uma rede viária de porte semelhante à do Reino Unido (cerca de 3,5 milhões de nós e 7,6 milhões de arestas), o pré-processamento pode levar em torno de uma hora em um único core, adicionando alguns milhões de atalhos e permitindo consultas de rota em dezenas de milissegundos para trajetos como Londres–Edimburgo.**

The query phase runs bidirectional Dijkstra with a crucial restriction: the forward search only follows edges to higher-ranked nodes, while the backward search only follows edges from higher-ranked nodes. Both searches rapidly climb the hierarchy, meeting at an important high-ranked node, typically a major highway junction. **This approach explores only hundreds of nodes instead of hundreds of thousands in large road networks, achieving order-of-magnitude speedups while maintaining optimality guarantees.**

Public descriptions of large-scale routing systems, including patents and engineering blogs do Google, indicam um desenho híbrido para integrar tráfego em tempo real. A porção inicial da rota usa busca com dados de tráfego em tempo real para máxima precisão, crítica para os primeiros minutos em que as condições mudam rapidamente. O restante pode usar Contraction Hierarchies com tráfego previsto a partir de padrões históricos, trocando precisão perfeita em tempo real pela velocidade necessária para atender milhões de consultas concorrentes. Em arquiteturas distribuídas, o grafo é particionado em múltiplos shards, com regiões sobrepostas para permitir processamento distribuído de consultas.
...
class ContractionHierarchies:
    ...
    def query(self, source: int, target: int) -> Tuple[float, List[int]]:
        ...
        # Backward search: only edges from higher-level nodes
        ...
        while forward_pq or backward_pq:
            ...
            if backward_pq:
                ...
                # Expand only from higher-level nodes
        ...
        return best_dist, []  # Path reconstruction omitted for brevity
...
The trade-off proves compelling for navigation: preprocessing takes minutes to hours but runs once (or periodically as roads change), while queries must complete in milliseconds millions of times per second. **Modern implementations typically incur apenas sobrecarga moderada de memória, e em alguns cenários representações comprimidas do grafo contraído podem até usar espaço semelhante ou menor que o grafo original com todas as arestas de baixo nível.** This effect occurs because the hierarchy allows efficient pruning of edges that would never be used in shortest-path queries.
...
## LSTM models capture temporal dynamics for real-time ETA prediction
...
Understanding the mathematical foundations separates developers who can use routing libraries from those who can build and optimize them. The shortest path problem formalizes navigation: given graph G = (V, E) with edge weight function w, find path P from source s to target t minimizing the sum of edge weights along P. **This elegant formulation reduces complex navigation to graph theory, where intersections become vertices, road segments become edges, and travel time or distance provides weights.**

Big-O complexity analysis explains why algorithm choice matters. Dijkstra's O((E + V) log V) seems fast, but on continental road networks with tens of millions of nodes, this still requires seconds. Contraction Hierarchies' O(log V) query time—exploring only hundreds of nodes—explains how Google Maps handles millions of simultaneous requests. **The key insight: preprocessing shifts computational burden from query time to setup time, and with billions of queries per day, even millisecond savings multiply into massive infrastructure cost reductions.**
...
Graph Convolutional Networks (GCNs) implement this using normalized adjacency matrices: H^(l+1) = σ(D^(-1/2) A D^(-1/2) H^(l) W^(l)), where A is the adjacency matrix, D is the degree matrix, W contains learnable weights, and σ is an activation function like ReLU. **This formulation spreads each node's information to neighbors, weighted by connection strength, and learns how to combine information through gradient descent on W.**
...
## Multimodal routing optimization introduces new algorithmic challenges

CityMapper's SuperRouter tackles a problem classical algorithms weren't designed for: simultaneously optimizing across multiple transportation modes—walking, biking, public transit, ride-sharing, scooters—each with different properties, costs, and availability. **The key challenge: combining schedule-based transportation (buses, trains with fixed departure times) with non-schedule-based modes (cars, bikes available on-demand) while handling real-time changes in a computationally tractable way.**

The system architecture uses layered graphs where each mode has its own network representation. Transit networks use route-based structures processing entire bus or train lines rather than individual stops, similar to academic RAPTOR algorithms. Road networks use traditional graph representations built from OpenStreetMap data enhanced with elevation, surface types, and accessibility information. **Floating transport modes like dockless bikes and scooters require dynamic layers that regenerate continuously as vehicle locations change.**

Transfer modeling proves critical and complex. Physical transfer costs include walking time between stations or stops, waiting time for the next service, and implicit comfort penalties. The system must validate transfers: checking whether walking distance between transfer points is reasonable, whether sufficient time exists given service frequencies, and whether floating vehicles exist at the transfer location with coverage extending to the destination. **CityMapper's competitive filtering shows multimodal combinations only when they provide genuine advantages over single-mode alternatives, avoiding suggestion overload.**

The query process runs different specialized algorithms for different mode types simultaneously. Transit routing uses a proprietary route-based approach processing each bus or train line once per search round. Walking and cycling use A* variants on the enhanced road network with elevation awareness for hill avoidance. Ride-sharing queries APIs for real-time availability and pricing. **These parallel searches connect through transfer nodes, with the overall system performing multi-criteria optimization balancing arrival time, number of transfers, cost, and user preferences.**

Real-time integration happens at near real-time, typically with sub-second latencies. GTFS (General Transit Feed Specification) data provides the base schedule, augmented with real-time service updates, disruptions, and delays. Uber's API supplies travel time estimates, waiting times, and pricing. Floating transport operators provide vehicle locations and coverage boundaries. CityMapper's BATCAVE toolset continuously validates and corrects this data, fixing issues from agencies, creating missing information, and managing real-time problems like strikes.

The system demonstrates sophisticated optimization by computing Pareto-optimal journeys—multiple non-dominated solutions where improving one criterion (like time) necessarily worsens another (like cost or transfers). The "Routing Powers" feature lets users apply real-time filters: TURBO mode calculates absolute fastest routes with real-time departures re-ranked every 60 seconds, or users can enable hill avoidance for cycling routes even if it adds time. **This personalization represents the evolution beyond simple shortest-path optimization toward user-centric multi-objective optimization.**

Performance examples highlight practical benefits. A Stanford to Apple HQ journey combining Caltrain and Uber saves 25 minutes and reduces cost from $40 to $10 versus cab-only. London Lancaster Gate to Gatwick Airport saves 17 minutes and £65 by mixing transit and ride-sharing. **These savings multiply across millions of daily trips, demonstrating why multimodal optimization matters beyond algorithmic elegance.**

## Production systems balance multiple competing constraints

Google Maps, Waze, and CityMapper reveal how theoretical algorithms transform into production systems handling planetary scale. Google Maps processes over 1 billion kilometers of daily driving, requiring distributed architectures that shard the road network graph across thousands of machines. **The hybrid routing approach uses Dijkstra with live traffic for immediate route portions where accuracy is critical, switching to Contraction Hierarchies with predicted traffic for distant segments where speed matters more than perfect real-time accuracy.**
...
Production systems already demonstrate this hybrid philosophy. Public information about Google Maps sugere o uso combinado de Contraction Hierarchies para explorar a estrutura do grafo e responder consultas rapidamente, A* com heurísticas para buscas focadas e Graph Neural Networks para previsão de tráfego. Waze reports combining A* routing on dynamically weighted graphs with LSTM-based ETA prediction. CityMapper describes specialized algorithms per mode while applying learned models to predict delays and demand patterns.
...
For practitioners, three principles emerge. First, no single algorithm solves all routing problems—production systems strategically combine classical methods, speedup techniques, and learned models based on each component's strengths. Second, mathematical foundations matter deeply; understanding complexity, optimality guarantees, and heuristic properties separates effective solutions from superficial applications. Third, real-world systems must address concerns far beyond algorithmic optimality—data quality, system reliability, computational cost, user experience, and privacy all shape production architecture decisions.

The comprehensive references provided—from Hart's 1968 A* paper to the 2021 DeepMind GNN deployment to current SIGSPATIAL research—offer pathways for deeper exploration. The Python implementations demonstrate that even sophisticated algorithms become accessible through careful study and implementation practice. The mathematical explanations aim to demystify complexity analysis, heuristic design, and neural architectures for developers without extensive mathematical backgrounds.

Navigation systems showcase computer science at its best: elegant theory, pragmatic engineering, and real-world impact converging to solve problems that matter. Whether optimizing personal commutes, coordinating emergency response, or enabling global logistics, routing algorithms form invisible infrastructure that modern society depends upon. Understanding how they work—from Dijkstra's relaxation to contraction hierarchies to graph neural networks—provides insight into both foundational computer science and the continuing evolution of intelligent systems.
...
- Introduction to Algorithms (Cormen et al.) - Wikipedia: https://en.wikipedia.org/wiki/Introduction_to_Algorithms
- Dijkstra's Algorithm in Python - GeeksforGeeks: https://www.geeksforgeeks.org/python/python-program-for-dijkstras-shortest-path-algorithm-greedy-algo-7/
- Guide to Dijkstra's Algorithm - Built In: https://builtin.com/software-engineering-perspectives/dijkstras-algorithm
...
- ETA Prediction with Graph Neural Networks - DeepMind: https://www.deepmind.com/publications/eta-prediction-with-graph-neural-networks-in-google-maps
...
- Graph Neural Network for Traffic Forecasting: A Survey - arXiv: https://arxiv.org/abs/2101.11174
- Graph Neural Network for Traffic Forecasting - MDPI: https://www.mdpi.com/2220-9964/12/3/100
