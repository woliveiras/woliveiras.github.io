...
Next, flash the OS onto your microSD card:

1. Open Raspberry Pi Imager on your computer.
2. Choose the OS (`Raspberry Pi OS Lite (64-bit)`) and the target microSD card.
3. Open advanced options: click the gear icon (or press `Ctrl+Shift+X`).
4. In Advanced Options:
  - Enable SSH and choose a password.
  - Set the **Hostname** to `raspberrypi` (keep it unique on your network).
  - Configure Wi‑Fi (SSID, password, country) if you will use wireless.
  - Set locale and keyboard as needed.
  - Change the default username.
5. Click Save, then Write to flash the card.

After the first boot, the Pi will use the configured hostname and SSH will already be enabled. Now you can connect the Pi to the power supply. The boot process takes about 1–2 minutes.
...
Running language models on small devices requires paying attention to memory usage. Models must be quantized in order to fit into the Raspberry Pi’s available RAM. For an 8 GB Raspberry Pi 5, a good starting point is a 1–3B parameter model.
...
Or, for an even lighter model:

```bash
ollama pull qwen2.5:1.5b
```

After the download finishes, run the model directly in your SSH session:

```bash
ollama run llama3.2:3b
```

Type a prompt such as “Explain what you are running on” and press Enter. When you are done, exit with `Ctrl+C`.
...
At the end of each response, Ollama reports the tokens-per-second rate. On a Raspberry Pi 5, you can expect:
