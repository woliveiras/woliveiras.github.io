---
title: "Building a Local LLM Server with Raspberry Pi 5, Ollama, and Tailscale"
description: "A complete and privacy-preserving home AI stack powered by modern ARM hardware."
pubDate: "Tue Nov 25 16:30:00 2025 +0200"
published: true
tags: ["llm", "ai-engineering", "hardware", "edge-ai", "ollama", "privacy", "tailscale", "raspberry-pi"]
---
...
This article is a hands-on tutorial for building a private, low-cost, always-available LLM server using a Raspberry Pi 5, the Ollama inference engine, and Tailscale for secure remote access. You will go from an empty microSD card to a fully working server that you can reach from your laptop or phone, even when you are outside your home network.
...
flowchart LR
    ...
    subgraph Network["Secure Network Path"]
        LAN["Home Network (Ethernet/Wi-Fi)"]
        TS["Tailscale VPN (WireGuard Tunnel)"]
    end

    subgraph Server["Raspberry Pi 5 - Local LLM Server"]
        OS["Raspberry Pi OS Lite (Headless)"]
        OLL["Ollama Inference Engine"]
        STORAGE["Model Storage<br/>MicroSD or NVMe"]
    end
...
Both approaches work; the remainder of this section explains how to flash with Raspberry Pi Imager and then how to migrate the running system from a microSD to the SSD using two reliable methods (automated `rpi-clone` or manual `rsync`).
...
Option A — rpi-clone (easy, recommended)

1. Install and run `rpi-clone` to copy the system from the SD to the SSD. **Replace `sda` with your SSD device name**:
```bash
cd ~
git clone https://github.com/billw2/rpi-clone.git
cd rpi-clone
sudo cp rpi-clone /usr/local/sbin/
# Confirm target device with lsblk, then run (replace sda):
sudo rpi-clone sda -f
```
2. Wait for the clone to finish. When done, power off the Pi, remove the microSD, and boot from the SSD.

Option B — rsync (manual, for advanced users or when you need to migrate an already configured system)
1. Prepare partitions on the SSD to match the Pi layout (FAT32 boot + ext4 root). Example using parted (replace `/dev/sda` with your SSD):
```bash
sudo parted /dev/sda -- mklabel gpt
sudo parted /dev/sda -- mkpart primary fat32 1MiB 256MiB
sudo parted /dev/sda -- mkpart primary ext4 256MiB 100%
sudo mkfs.vfat -F32 /dev/sda1
sudo mkfs.ext4 /dev/sda2
```
2. Mount the new partitions and copy the filesystem with `rsync`:
```bash
sudo mkdir -p /mnt/ssd-boot /mnt/ssd-root
sudo mount /dev/sda2 /mnt/ssd-root
sudo mount /dev/sda1 /mnt/ssd-boot
sudo rsync -aAXv --exclude={'/dev/*','/proc/*','/sys/*','/tmp/*','/run/*','/mnt/*','/media/*','/lost+found'} / /mnt/ssd-root
sudo cp -a /boot/* /mnt/ssd-boot/
```
3. Update `fstab` and `cmdline.txt` on the SSD to point to the SSD partitions (use `blkid` to get UUIDs or PARTUUIDs):
```bash
sudo blkid
sudo nano /mnt/ssd-root/etc/fstab    # replace old UUIDs with the SSD UUIDs
sudo nano /mnt/ssd-boot/cmdline.txt  # update root=PARTUUID=... to point to SSD root
```
4. Unmount, power off, remove microSD, and boot the Pi from the SSD.

Verification after booting from SSD:
```bash
lsblk
mount | grep ' / '
df -h /
...
On a Raspberry Pi 5, you can expect:

- Around 5–6 tokens/second for ~3B models (fluid terminal experience).
- 8–10+ tokens/second for ~1B models (very responsive).
- 2–3 tokens/second for 7–8B models (usable for automations, slower for chat).

These numbers depend on cooling, power stability, and whether you are using NVMe or microSD for storage.

## Step 5 – Connect from Your Laptop or Phone

Once the server is configured, you can interact with it from other devices.

From another machine on your home network or Tailnet, test the REST API with `curl`:
