...
Every engineering team agrees that documentation matters. Almost no engineering team writes enough of it. The reason is simple economics: documentation is high-value but low-urgency. It rarely blocks the current sprint, so it lives at the bottom of the backlog. Meanwhile, the codebase grows, new people join, and everyone pays the cost of tribal knowledge.

AI changes the economics because documentation has a lot of structured, repetitive work: describe parameters, summarize endpoints, extract commands, produce examples, and turn a diff into a PR narrative. That makes it a good delegation target.

But the review bar is higher than most teams apply. Generated documentation often sounds confident while describing behavior the code does not have. Treat generated documentation as untrusted output, the same way you treat generated code. The assistant can draft the words. You own the accuracy.
...
Documentation is a natural fit for the delegation-review pattern: you delegate the writing to the agent, review for accuracy and completeness, and correct where the agent hallucinated or missed context. Documentation errors are often easier to spot than code bugs, but they are not harmless. A wrong README command wastes onboarding time. A wrong API contract breaks client teams. A wrong security note can create a production incident.
...
That "why" is where senior documentation lives. AI can infer what a function does from the body. It cannot know why the team accepted a trade-off, why a migration had to be reversible, or why a legacy endpoint cannot change shape until a mobile app version ages out.
...
My review rule for generated docs is strict: every command must be runnable, every path must exist, every environment variable must appear in a real config or example file, and every claim about behavior must be traceable to code, tests, or a decision record.
...
For running this exercise, you'll need a project with some undocumented code (or use the `PaymentProcessor` from Chapter 12).
...
| Step | What happened |
|---|---|
| **Setup** | You assessed the documentation gap and created a documentation standard |
| **Generation** | You generated docs for functions, types, and the module README |
| **Review** | You checked each generated doc against the actual code |
| **Correction** | You fixed inaccuracies, added missing "why" context, and updated your documentation instructions |
...
This way, both Copilot Code Review (Chapter 11) and human reviewers enforce documentation standards automatically.
...
1. **AI makes documentation cheap enough to do at the right time.** Use it to draft the repetitive structure, then review every claim against the code and add the human context.
