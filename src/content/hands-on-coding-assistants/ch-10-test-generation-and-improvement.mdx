...
## Welcome to Module 2: spec-first in daily practice

Chapters 0 through 4 covered the foundations — prerequisites, the spec-first workflow, setup, and the harness overview (Module 0). Chapters 5 through 9 gave you the complete toolkit: prompt engineering, custom agents, AGENTS.md, repository instructions, and skills (Module 1). Starting now, Module 2 puts that foundation to work on the tasks that deliver the most value in daily engineering. We begin with the single highest-impact use case for AI coding assistants: **test generation**.

---

## Why test generation is the #1 productivity use case

Test generation is where AI assistance has one of the cleanest engineering payoffs. Not because generated tests are automatically good. They are not. The payoff is that tests give you an executable way to challenge generated implementation code.
...
2. **Tests are the easiest AI output to verify.** You can run a test suite and get a binary answer: it passes or it fails. Compare that to generated application code, where correctness is harder to judge without manual review. The review step is fast and unambiguous.
...
Test generation is a natural fit for the spec-first workflow: you delegate the test writing to the agent, review by running the suite, and iterate on gaps. The spec is lightweight here — you already know what code exists and what coverage is missing.

This fits the broader research pattern from the series research sources: AI works best on constrained tasks with clear verification. A generated test is still untrusted output, but it is easier to inspect than a generated architecture decision. You can read the assertions, run the suite, and reject tests that only chase coverage.
...
**Start with the easy wins.** Generate unit tests first. Move to integration tests once the unit layer is solid. Tackle E2E tests only when you have clear specs and a running environment.
...
Decide your mock boundaries before delegating to the agent. The clearer the boundary, the more useful the generated tests.
...
My standard for AI-generated tests is stricter than my standard for hand-written first drafts: every test must fail for a meaningful bug. If I cannot name the bug a test would catch, I delete or rewrite the test.
...
### 5. Tests that encode the agent's wrong assumption

The most dangerous generated test is one that makes incorrect behavior look official. This happens when the agent invents a requirement, then writes a test that locks it in.

```typescript
// The spec never said admins bypass rate limiting.
it("allows admins to bypass rate limiting", () => {
  expect(canBypassRateLimit({ role: "admin" })).toBe(true);
});
```

**Fix:** Review tests against the spec before reviewing the implementation. Generated tests are not the source of truth. The spec is.
...
In Ch 9, we built Agent Skills for specialized tasks. A testing skill is a natural fit:
...
| **Accepting tests without reading them** | Weak assertions and missing edge cases slip through | Always review AI-generated tests before merging |
...
1. **Test generation is a high-leverage use case because it has a fast feedback loop.** You can run the tests, inspect the assertions, and reject weak coverage immediately. No other AI use case gives you a tighter check on generated output.
...
6. **Always review.** AI-generated tests that pass aren't necessarily good tests. Check for meaningful assertions, independent tests, and coverage of error paths. Reviewing agent output is non-negotiable.

7. **Enforce a coverage floor, not a fixed threshold.** Configure your CI to fail if coverage _drops_ below its current value — not just below an arbitrary number. This creates a ratchet: coverage can only go up. When the agent generates new code, it must also generate the tests, otherwise the floor falls and CI blocks the merge:

   ```typescript title="vitest.config.ts (coverage ratchet pattern)"
   coverage: {
     thresholds: {
       autoUpdate: false,  // update manually after deliberate improvements only
       lines: 78,          // set to your current baseline
       functions: 82,
       branches: 71,
     }
   }
   ...
   The discipline: only update the threshold numbers when you've intentionally improved coverage. Don't auto-update to make CI green — that defeats the ratchet.

In Ch 11, we'll apply the same Review-focused mindset to **code review** — how Copilot Code Review works, how to customize it with instructions, and how to let it automatically review every PR your team opens.
