...
Chapters 0 through 5 covered the foundations and the Plan phase — prerequisites, the PDRC mental model, setup, Spec-Driven Development, and prompt engineering (Module 0). Chapters 6 through 9 gave you the complete customization toolkit: custom agents, AGENTS.md, repository instructions, and skills (Module 1). Starting now, Module 2 puts that foundation to work on the tasks that deliver the most value in daily engineering. We begin with the single highest-impact use case for AI coding assistants: **test generation**.
