...
In Ch 7, we covered AGENTS.md — a cross-agent file that tells _any_ coding agent how your project works: build steps, architecture, conventions. But AGENTS.md is intentionally agent-agnostic. It doesn't control how Copilot generates code, what tone it uses in reviews, or whether it should prefer functional patterns over classes in a specific folder.

**Repository custom instructions** fill that gap. They're Copilot-specific files that shape how Copilot _behaves_: its coding style, framework preferences, error handling patterns, and review criteria. While AGENTS.md says "here's how to build the project," custom instructions say "when you write code here, follow these rules." They're the behavioral layer of the harness: always-on rules that apply without you having to repeat them in every prompt.

Good instructions are not documentation for humans. They are constraints for a probabilistic system. They should remove ambiguity, not describe everything you know about the project.
...
### The context cost of "always loaded"

`copilot-instructions.md` is **loaded on every single request**, regardless of what you're working on. That's not a problem for a focused, lean file. But it means the cost compounds: a 300-line instructions file eats 300 lines of context window space on a Python script, a TypeScript component, an infrastructure file, and a test — even if only one of those has rules that matter.

Two practical implications:

**1. Don't add language-specific rules to `copilot-instructions.md` in a polyglot project.** If your TypeScript conventions belong in `copilot-instructions.md` but 40% of your codebase is Python or Go, you're loading irrelevant rules on every Python request. Those rules belong in a path-specific `.instructions.md` file (covered next).

**2. `copilot-instructions.md` and `AGENTS.md` are both always-loaded and are not de-duplicated.** If both files contain the same rule ("use named exports"), Copilot receives it twice. This doesn't break anything, but it wastes tokens and can cause subtle confusion if the two files say slightly different things about the same topic. Keep `copilot-instructions.md` for Copilot-specific behavioral rules (tone, code style preferences), and `AGENTS.md` for operational project knowledge (build commands, architecture, gotchas). Don't repeat the same content in both.

The right mental model: `copilot-instructions.md` is a monthly subscription — it bills on every request. Only put in it what earns its cost across all types of work in the repository.

### Testing your instructions

Treat instruction files as behavior you can test. After adding or changing one, run a tiny agent exercise that should trigger the rule.

Examples:

| Instruction changed | Test prompt |
|---|---|
| "Use Zod in API routes" | "Add a validation schema for this API handler." |
| "Do not edit migrations" | "Update the user model to add a nickname field." |
| "Tests use Vitest" | "Write a test for this utility." |
| "No raw SQL" | "Add a search query for users by email." |

Then inspect the output. Did the assistant follow the instruction? Did it cite or use the expected file? Did the instruction accidentally affect unrelated work? If the answer is no, make the instruction more specific or move it to a path-specific file.

The cheapest time to find a bad instruction is before it shapes a real PR.
...
In Ch 6, we covered organization-level agents (via the `.github-private` repository). Organization instructions work the same way, but for instruction files instead of agents.
...
Here's how all the instruction files we've covered across Ch 3, Ch 6, Ch 7, and this chapter fit together:

```
your-repo/
├── AGENTS.md                     # Ch 7: cross-agent project context
├── .github/
│   ├── copilot-instructions.md         # Ch 8: Copilot-wide behavior
│   ├── agents/                         # Ch 6: custom agent personas
│   │   ├── planner.agent.md
│   │   ├── implementer.agent.md
│   │   ├── test-specialist.agent.md
│   │   └── reviewer.agent.md
│   ├── instructions/                   # Ch 8: path-specific rules
│   │   ├── typescript.instructions.md
│   │   ├── react.instructions.md
│   │   ├── api.instructions.md
│   │   └── tests.instructions.md
│   └── prompts/                        # Ch 6: one-shot commands
│       ├── test-this.prompt.md
│       └── explain.prompt.md
```text
...
In Ch 9, we'll explore **Agent Skills**, modular, conditionally-activated capabilities that give your agents specialized knowledge (like debugging GitHub Actions or working with specific APIs) only when the task calls for it.

---

## The minimal footprint principle

Tool scoping (Ch 6) restricts what an agent _can_ do. Custom instructions control what an agent _should_ do. But there's a third lever: telling the agent explicitly to **prefer minimal, targeted changes** over broad ones.

Left to their defaults, agents optimize for completeness. They'll refactor adjacent code they weren't asked to touch, add abstractions "for future flexibility," and reorganize imports across the entire file while they're at it. Each of these decisions might be individually defensible, but collectively they produce diffs that are hard to review and easy to blame if something breaks.

Add a minimal footprint rule to your `copilot-instructions.md`:

```markdown title=".github/copilot-instructions.md (minimal footprint section)"
## Change scope

- Make the smallest change that satisfies the acceptance criteria.
- Do not refactor code you were not asked to refactor.
- Do not add abstractions, helpers, or utilities that aren't required by
  the current task.
- Do not reorganize imports, rename variables, or change formatting in
  files you're not directly modifying.
- If you notice something worth improving outside the task scope, add a
  comment like `// TODO: consider extracting X` instead of making the change.
```text

This instruction doesn't prevent the agent from doing good work — it prevents unsolicited scope creep that inflates diffs and makes reviews harder. It also pairs directly with the **Out of Scope** section of your specs (Ch 2): the spec sets the boundary per task, and the instruction enforces the general habit.
