...
> **Note on plans:** GitHub Copilot Free gives you a monthly limit of inline suggestions and chat interactions. Copilot Pro removes that limit and adds premium request quotas for advanced models. Copilot Pro+ gives higher quotas. For this workshop, the Free plan works, but you'll hit limits faster during hands-on exercises. Check the [current plans](https://github.com/features/copilot/plans) for the latest pricing and limits.
...
In Ch 4, we'll dive into **prompt engineering** — how to structure your prompts, manage context, decompose complex tasks, and know when _not_ to use AI. In Ch 5, we'll build **custom agents and sub-agents** with `.agent.md` profiles that turn your prompt patterns into reusable team knowledge.
