...
description: "Complete challenge applying the full spec-first workflow with multi-agent collaboration on a real codebase."
module: 5
order: 18
---
...
This is the chapter where everything connects. Across Modules 0-4, you've learned the prerequisites and foundations, customized agents and built the harness, applied the spec-first workflow to daily engineering tasks, extended the system with MCP, hooks, and automation, and governed it with security controls and measurement.

Now you'll run the complete workflow on a single realistic project. No new concepts. No new tools. Just the disciplined application of everything you've practiced, measured against the criteria you define.

The chapter is structured as a guided project with explicit checkpoints. You can work through it on any codebase — your own, a side project, or the sample scenario provided below. The goal isn't the specific code. It's the workflow: **write a spec → Plan mode → implement → review against criteria → correct** with evidence at every step.

Run this project in small batches. The final artifact should not be one giant agent diff that nobody wants to review. Each checkpoint should produce a clear state: what changed, which acceptance criteria are satisfied, which commands ran, and what risk remains.
...
Your job: take this issue through the full spec-first workflow using AI-assisted development, as if you were onboarding your team to this workflow for the first time.
...
This is where the developer thinks. The AI doesn't write code yet. You do the design work that makes delegation effective.

### Step 1.1: write the specification

Transform the issue into a proper spec using the template from Ch 2. The spec should be detailed enough that someone (or something) unfamiliar with the codebase could implement the feature correctly.
...
```yaml title=".github/agents/planner.agent.md"
---
name: planner
...
```yaml title=".github/agents/implementer.agent.md"
---
name: implementer
...
```yaml title=".github/agents/test-writer.agent.md"
---
name: test-writer
...
Before moving to Plan mode, verify:
...
- What changed in the original issue?
- Which decisions did you make?
- Which items are explicitly out of scope?
- What would make this task unsafe to delegate?

---

## Phase 2: Plan mode → implement

Now the AI writes code. Your job shifts from author to supervisor.

### Step 2.1: use Plan mode first

Before assigning the issue to the coding agent, use Plan mode in VS Code Chat (or `Shift+Tab` in Copilot CLI) to get the agent's implementation approach:

> Switch to Plan mode and paste the spec. Review: which files will change? In what order? Does the plan match your technical notes?

If the plan looks wrong — wrong files, wrong order, wrong approach — correct it now. This is the cheapest point to catch a misunderstanding. Once you're satisfied with the plan, proceed.

### Step 2.2: delegate to the coding agent
...
### Step 2.3: monitor — don't micromanage

While the agent works, resist the urge to intervene at every step. The whole point of writing the spec and reviewing the plan was to give the agent clear enough instructions to work autonomously.
...
For each checkpoint, ask for evidence:
...
### Step 2.4: multi-agent collaboration (optional)
...
**Checkpoint 2: Implementation evidence**

Record:

- Migration created and rollback verified
- Service/model slice implemented
- API slice implemented
- Tests added or updated
- Full verification command and result
- Any command that failed before passing
...
If you've configured Copilot Code Review (Ch 11), it runs automatically on the PR. Check what it found:
...
This is your measurement instrument from Ch 2 in action. The spec told the agent what to do, and now it tells you exactly what was missed.
...
### Before moving to correction, you should have:
...
- Automated review findings accepted, rejected, or deferred
- Security findings and mitigation
- Manual review notes that require human judgment

---

## Phase 4: Correct and iterate
...
**Checkpoint 4: Correction complete**

Record:

- Criteria fixed in the correction round
- New tests added because of review
- Remaining risks, if any
- Final commands run before merge

---

## Phase 5: Retrospective

The workflow for this task is complete. Now step back and evaluate the process itself.
...
These are the moments the spec-first workflow is designed to protect. The developer's role isn't typing code — it's making decisions and verifying outcomes.
...
**Month 3: Measurement**
- Establish your full measurement dashboard (Ch 17)
...
| Pitfall | Symptom | Fix |
| --- | --- | --- |
| Skipping the spec | High correction rounds, scope creep in PRs | Require specs for all agent-assigned issues |
...
| No security review | Secrets in PRs, overly permissive agent access | Run the Ch 16 security audit checklist |

### The role of the developer, revisited

In Ch 1, we said the central idea of this series is: **you stay in control, and the AI amplifies your decisions.** After 18 chapters, that idea should feel concrete:
...
| --- | --- | --- |
| **0 — Foundations** | 0-4 | Prerequisites, spec-first workflow, setup, the harness |
| **1 — Agent Customization** | 5-9 | Prompt engineering, custom agents, AGENTS.md, repository instructions, skills |
| **2 — AI in the Daily Cycle** | 10-13 | Test generation, code review, debugging & refactoring, documentation |
| **3 — Beyond Code** | 14-15 | MCP for external tools, hooks for automation, issue-to-PR pipelines |
| **4 — Trust at Scale** | 16-17 | Security, governance, firewalls, IP, impact measurement |
| **5 — Full Cycle** | 18 | End-to-end application of everything, retrospective, team adoption |

### The spec-first workflow — one last time

| Step | What you do | What the AI does |
| --- | --- | --- |
| **Write spec** | Define acceptance criteria, technical notes, out of scope | Analyze the codebase, flag risks, suggest implementation plans |
| **Plan mode** | Review and approve the implementation approach | Build a structured plan, ask clarifying questions |
| **Implement** | Assign the task, monitor at checkpoints | Write code, run tests, create PRs |
| **Review** | Check spec compliance, evaluate quality, assess security | Run automated reviews, highlight potential issues |
| **Correct** | Provide specific feedback, update the harness, record lessons | Iterate on feedback, push fixes, re-run tests |

The cycle repeats. Each iteration improves the instructions, the agents, and the specs — which improves the next cycle. This is the continuous improvement loop from Ch 17, and it's the real output of this series: not any single piece of code, but a system that gets better every time you use it.

---

## Final hands-on: your full workflow
...
2. **Write a spec.** Use the template from Ch 2: acceptance criteria, technical notes, and out-of-scope section. Use the planner agent (or Copilot Chat) to validate the approach and flag ambiguities. Resolve all ambiguities before proceeding.

3. **Plan mode.** Switch to Plan mode. Review the agent's implementation approach against your spec. Approve when it looks correct.

4. **Implement.** Assign the task to the coding agent (or switch to Build mode). Let the agent complete the implementation without intervention unless it's stuck.

5. **Review.** Score the PR against your spec (criteria met / total criteria). Run the security checklist from Ch 16. Note every issue you find.

6. **Correct.** Write specific review comments for each issue. Let the agent iterate. Re-review and score again.

7. **Retrospect.** Write a retrospective covering: timeline, spec compliance (before and after), what worked, what needed intervention, what you'd change, and metrics.

8. **Improve the harness.** Based on the retrospective, make at least one change to your custom instructions, agent profiles, or spec template. Commit it to the repository.
...
- [ ] At least one harness improvement committed
...
The tools will keep evolving. Models will get smarter. Agents will become more autonomous. New features will ship every month. But the spec-first workflow doesn't depend on any specific tool or model. It depends on the fundamental principle that drove every chapter: **the developer who writes clear specs, uses Plan mode, builds a solid harness, and reviews rigorously will always get better results than the developer who just types a prompt and hopes for the best.**

If you want a head start on the harness for your next project, [Geremmyas](https://woliveiras.com.br/posts/introducing-geremmyas-spec-driven-development-ai-assistants/) installs a production-ready version of everything from Modules 0–1 in a single command.
