...
Transform the issue into a proper spec using the template from Ch 4. The spec should be detailed enough that someone (or something) unfamiliar with the codebase could implement the feature correctly.
...
This is your measurement instrument from Ch 4 in action. The spec told the agent what to do, and now it tells you exactly what was missed.
...
4. **Review.** Score the PR against your spec (criteria met / total criteria). Run the security checklist from Ch 16. Note every issue you find.
