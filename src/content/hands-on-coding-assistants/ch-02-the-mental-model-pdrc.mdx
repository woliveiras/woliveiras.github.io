...
description: "The way I think about working with coding agents — a personal adaptation of the PDCA cycle for AI-assisted development."
module: 0
order: 2
---

## Why you need a way of thinking about this at all
...
What I'm about to share isn't a formal methodology or an industry standard. It's the way I personally learned to work with coding agents after a lot of trial, error, and wasted time. I call it **PDRC** — Plan, Delegate, Review, Correct. It's heavily inspired by a well-known quality cycle (more on that below), but adapted to the specific reality of working with AI tools every day. Take what works for you, adjust what doesn't, and make it your own.

---

## The inspiration: PDCA
...
**PDRC is my personal adaptation of PDCA for AI-assisted software development.** It's not an official extension of Deming's work or a formally validated methodology — it's just the names I found most useful for describing what actually happens when you work with a coding agent.
...
1. **What's the task?** — Be specific. "Add authentication" is not a plan. "Add JWT-based authentication middleware that validates tokens from our auth service, returns 401 for expired tokens, and passes the decoded user object to downstream handlers" is a plan (abbreviated for brevity).
...
For anything beyond a trivial task, the output of the Plan phase should be a concrete artifact — a structured document that defines the goal, the acceptance criteria, and the boundaries of the task before any tool is opened. In Ch 4, we'll build this practice into a repeatable workflow called Spec-Driven Development.
...
graph TD
    ...
    Q -->|Yes| Done["✅ Accept & commit"]
    Q -->|Almost| RP["Re-prompt with context"]
    Q -->|Partially| MI["Fix manually + re-prompt the rest"]
    Q -->|No| D{"Fixable?"}
    D -->|Yes, but extensive| Restart["Discard → restart with better plan"]
...
PDRC is a spectrum, not a checkbox. Use your judgment — but _have_ a mental model for how you engage with AI. The alternative is "accept all," and we've seen where that leads.
...
|--------|-------|---------------|
| **Module 00** (Ch 3–5) | Setup, spec-driven development, prompt engineering | **Plan** — configuring tools, specifying tasks, and learning to prompt effectively |
| **Module 01** (Ch 6–9) | Custom agents, AGENTS.md, instructions, skills | **Plan** — teaching the AI your context |
| **Module 02** (Ch 10–13) | Tests, code review, debugging & refactoring, documentation | **Delegate + Review** — effective tasking and output evaluation |
| **Module 03** (Ch 14–15) | MCP integrations, hooks, agent automation | **Delegate** — extending what the AI can access and do |
| **Module 04** (Ch 16–17) | Security, governance, impact measurement | **Review** — systematic verification and risk management |
| **Module 05** (Ch 18) | End-to-end final project | **Correct** — continuous improvement based on data |

You'll apply PDRC in every hands-on exercise. By the end of the series, it won't be something you _think about_ — it'll be muscle memory.
