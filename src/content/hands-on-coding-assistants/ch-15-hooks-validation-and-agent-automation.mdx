...
Any hook type that fires before or during a turn can return an `additionalContext` field. The agent receives this text as extra context alongside its current task — without you having to type it into the chat.
...
The hook lifecycle and what each stage can inject:

| Hook | `additionalContext` | `decision: "block"` | Notes |
|---|---|---|---|
| `sessionStart` | Before first turn | No | Best for dynamic project state |
| `userPromptSubmit` | Alongside the prompt | Yes (blocks prompt) | Useful for auto-attaching context |
| `postToolUse` | After tool result | Yes (replaces tool result) | Run linters, inject failure details |
| `Stop` (session end) | Continue session with reason | Yes (continues session) | Re-trigger if tests fail |
| `SubagentStart` (VS Code) | Into sub-agent's context | No | Sub-agents get their own injection |
| `PreCompact` (VS Code) | Before compaction | No | Export critical context before truncation |

### The `PreCompact` hook: your last chance before the context shrinks
...
The `PreCompact` hook fires **immediately before compaction**. It's the only point in the lifecycle where you can export or preserve context that would otherwise be summarized away. Use it to write critical state to a file, post a summary to an external system, or log what the agent was working on:
