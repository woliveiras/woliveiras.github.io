...
Ch 14 introduced MCP, the protocol that lets Copilot talk to external systems. MCP answers the question "what can the agent access?" This chapter answers the next question: **"what happens before, during, and after the agent acts?"**
...
Hooks are the enforcement layer of the harness. Instructions tell agents what to do; hooks enforce what they can do. Instead of reviewing agent output manually after the fact, hooks inject automated checks that run *during* agent execution.

This distinction matters. An instruction is a request to the model. A hook is code. When the risk is high, prefer code.
...
### The GitHub Copilot cloud/CLI hook triggers
...
| `agentStop` | The main agent is about to stop | Run final validation or collect completion evidence |
| `subagentStop` | A subagent is about to stop | Collect delegated task output or diagnostics |
...
VS Code also has a hooks preview with PascalCase event names such as `SessionStart`, `UserPromptSubmit`, `PreCompact`, `SubagentStart`, `SubagentStop`, and `Stop`. Treat that as a separate surface: don't put a VS Code preview event name in a GitHub cloud/CLI `.github/hooks/*.json` file and expect it to run there.
...
### Output: injecting context into the agent

`preToolUse` blocking is the most visible hook capability, but **context injection** is the most powerful one for improving agent quality.

Any GitHub cloud/CLI hook type that fires before or during a turn can return an `additionalContext` field. The agent receives this text as extra context alongside its current task — without you having to type it into the chat.

```json
{
  "hookSpecificOutput": {
    "hookEventName": "sessionStart",
    "additionalContext": "Current branch: feature/auth-refactor | Node: v22.3.0 | Last commit: fix: resolve token expiry race condition"
  }
}
```

The GitHub cloud/CLI hook lifecycle and what each stage can inject:

| Hook | `additionalContext` | `decision: "block"` | Notes |
|---|---|---|---|
| `sessionStart` | Before first turn | No | Best for dynamic project state |
| `userPromptSubmitted` | Alongside the prompt | No | Useful for auto-attaching context |
| `postToolUse` | After tool result | No | Run linters, inject failure details |
| `agentStop` | Before the main agent stops | No | Capture final evidence before completion |
| `subagentStop` | Before a subagent stops | No | Capture delegated task output |

### VS Code preview: the `PreCompact` hook

When the context window fills up (around 95% capacity), the agent automatically compacts the conversation — it summarizes older turns to make room for new work. Important details get lost in that summarization.

In the VS Code hooks preview, the `PreCompact` hook fires **immediately before compaction**. It's the point in that lifecycle where you can export or preserve context that would otherwise be summarized away. Use it to write critical state to a file, post a summary to an external system, or log what the agent was working on:

```sh title=".github/hooks/scripts/pre-compact.sh"
#!/bin/bash
INPUT=$(cat)
SESSION_ID=$(echo "$INPUT" | jq -r '.sessionId')
TRANSCRIPT=$(echo "$INPUT" | jq -r '.transcript_path')

# Save a checkpoint summary before the context is compacted
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
cp "$TRANSCRIPT" "./logs/session-checkpoint-${TIMESTAMP}.jsonl"
echo "Checkpoint saved before compaction: $TIMESTAMP" >> ./logs/compaction.log
```

This is particularly valuable for **long-running agentic sessions** — multi-hour sessions building a feature, running a full test suite, or coordinating multiple sub-agents. Without a PreCompact hook, you may find the agent has "forgotten" early context by the time it reaches the final steps.

Do not add `PreCompact` as a trigger in a GitHub cloud/CLI `.github/hooks/*.json` file. Keep it in the VS Code hooks configuration surface that supports preview lifecycle events.

### The `sessionStart` hook: dynamic context injection

The most high-value use of `additionalContext` is in the `sessionStart` hook. This fires once, before the agent reads any file or takes any action — making it the ideal injection point for dynamic project state that AGENTS.md can't capture (because AGENTS.md is static).

```sh title=".github/hooks/scripts/inject-session-context.sh"
#!/bin/bash
BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
NODE_VER=$(node -v 2>/dev/null || echo "unknown")
LAST_COMMIT=$(git log -1 --oneline 2>/dev/null || echo "no commits")
FAILING_TESTS=$(npx vitest run --reporter=dot 2>&1 | grep -c "FAIL" || echo "0")

CONTEXT="Branch: $BRANCH | Runtime: Node $NODE_VER | Last commit: $LAST_COMMIT"
if [ "$FAILING_TESTS" -gt 0 ]; then
  CONTEXT="$CONTEXT | WARNING: $FAILING_TESTS failing tests"
fi

echo "{\"hookSpecificOutput\":{\"hookEventName\":\"sessionStart\",\"additionalContext\":\"$CONTEXT\"}}"
```

The agent now begins every session knowing the current branch, runtime version, last commit, and whether tests are passing — without you having to type any of it. This is the right complement to AGENTS.md: the file holds what never changes (build commands, architecture, conventions); the hook injects what changes every session (state).
...
**One critical formatting rule**: hook output must be compact single-line JSON. Multi-line JSON will fail to parse. Use `jq -c` in Bash or `ConvertTo-Json -Compress` in PowerShell when constructing JSON output.

---

## Practical hook patterns

Let's walk through four patterns that solve real engineering problems.

### Pattern 1: security guardrails
...
**Why this matters**: the agent has access to a bash shell. Without guardrails, a misinterpreted prompt could lead to destructive commands. This hook adds an automated enforcement layer that catches dangerous patterns before they execute. You can customize the regex to match your project's specific risks (for example, blocking `aws s3 rm` commands in a cloud project).

Do not make this hook clever. Make it conservative, predictable, and easy to review. Security hooks that rely on subtle parsing become another thing the agent can accidentally route around. Block obvious destructive patterns, log suspicious commands, and require human approval for anything with production blast radius.

### Pattern 2: file boundary enforcement
...
This enforces the boundaries described in your instruction files (Ch 8) at the infrastructure level. Instructions tell the agent what it *should* do; hooks enforce what it *can* do.

Boundary hooks are especially useful for specialized agents. A docs agent should not edit migrations. A test writer should not modify production code. A reviewer should not write at all. The same rule should appear in the agent profile and in the enforcement hook when the cost of violation is high.
...
Audit logs are also your improvement data. When the same command is blocked repeatedly, either the hook is too strict or the agent instructions are too vague. Feed that signal back into the harness instead of dismissing it as noise.
...
The same pattern works for Microsoft Teams, PagerDuty, or any service that accepts webhook calls. The agent delegates error notifications to your team's communication channels without any manual action.
...
The `environment: copilot` line gives the job access to secrets stored in the Copilot environment (the same one you set up for MCP in Ch 14).
...
**Azure MCP authentication (from Ch 14):**
...
This connects to Ch 14's Azure MCP example. The Azure login step runs before the agent starts, so the Azure MCP server can authenticate via the Azure CLI session.
...
| **Implementation** | Agent reads the issue, plans, writes code, runs tests | Custom instructions (Ch 8), MCP (Ch 14) |
...
| **Automated review** | Copilot Code Review (Ch 11) reviews the PR | Repository settings |
...
3. `.github/copilot-instructions.md` — agent instructions (Ch 8).
4. MCP configuration in repository settings (Ch 14).
5. Copilot Code Review enabled in repository settings (Ch 11).
...
5. Review the PR. Copilot Code Review (Ch 11) should have already posted its review.
...
3. **Security hooks enforce what instructions recommend.** Custom instructions (Ch 8) tell the agent what it *should* do. `preToolUse` hooks enforce what it *can* do. Use both layers together.
...
## Agentic Workflows: repository automation in Markdown

Hooks give you control over what happens _during_ an agent session. Agentic Workflows go one step further: they let you define fully automated multi-step processes that run on GitHub Actions triggers, written in plain Markdown — not YAML.

Launched in technical preview in February 2026, **Agentic Workflows** are the bridge between the customization work you've done in Modules 1 and 3 and true repository-level automation. Where hooks react to agent events, Agentic Workflows define entire orchestration sequences: triage incoming issues, generate a PR from a spec, run the test suite, and ping the reviewer — all without writing a single YAML workflow file.

### What Agentic Workflows are

An Agentic Workflow is a `.workflow.md` file stored in `.github/workflows/` (or `.github/agents/`). It contains:

- A **trigger**: which GitHub event starts the workflow (issue opened, PR commented, label added, schedule, etc.)
- A **goal**: what the workflow should accomplish, in natural language
- **Steps**: a sequence of tasks delegated to the AI agent
- **Constraints**: rules the agent must follow (tool access, file boundaries, exit conditions)

The agent executes the steps using the same tools it has in a normal session: read/write files, run shell commands, call MCP servers, create PRs.

### Example: auto-triage incoming issues

This workflow runs every time a new issue is opened. The agent reads the issue, classifies it, adds appropriate labels, and leaves a comment acknowledging receipt.

```markdown title=".github/workflows/triage.workflow.md"
---
trigger:
  event: issues
  types: [opened]
tools: [read, search, github/add_labels, github/create_issue_comment]
---

# Issue Triage Workflow

## Goal
Classify and label new issues when they're opened, then acknowledge receipt.
...
2. Classify the issue into one of these categories:
   ...
   - `question` — needs clarification or documentation
   - `improvement` — enhancement to existing functionality
   - `chore` — maintenance, dependency update, refactor

3. Add the matching label to the issue.

4. If the issue is a `bug`, also check if it matches any open issues.
   If a duplicate exists, add the `duplicate` label and reference the
   original in a comment.

5. Leave a comment on the issue:
   - For `bug`: confirm it's been triaged and ask for reproduction steps
     if none were provided.
   - For `feature-request`: acknowledge and mention the next review cycle.
   - For all others: a brief acknowledgment.

## Constraints
- Only modify the issue you were triggered on. Do not touch other issues.
- Do not create new labels — only use existing ones.
- Do not assign the issue to anyone.
```

### Example: spec-to-PR automation

This workflow triggers when an issue with the label `agent-ready` is opened or labeled. The agent reads the spec in the issue body, implements it, and opens a PR.

```markdown title=".github/workflows/implement-spec.workflow.md"
---
trigger:
  event: issues
  types: [labeled]
  filters:
    label: agent-ready
tools: [read, search, edit, execute, github/create_pull_request]
---

# Implement Spec Workflow

## Goal
Implement the feature spec in the issue body and open a pull request.

## Steps

1. Read the issue body. Locate the Goal, Acceptance Criteria, Technical
   Notes, and Out of Scope sections.

2. Use the Planner agent to generate an implementation plan. Review it
   for accuracy before proceeding.

3. Implement each acceptance criterion in order. Run existing tests after
   each significant change.

4. Write new tests covering the acceptance criteria.

5. Run the full test suite. All tests must pass before creating a PR.

6. Create a pull request:
   - Title: the issue title
   - Body: paste the spec + a summary of implementation decisions
   - Reference the issue with "Closes #<issue-number>"
   - Request review from the team's code-review group

## Constraints
- Follow `.github/copilot-instructions.md` strictly.
- Only create files specified in Technical Notes or directly required
  by the acceptance criteria.
- Do not merge the PR — only create it.
```

### How Agentic Workflows relate to what you've already built

Agentic Workflows are an orchestration layer on top of everything in this module:

| What you built | What Agentic Workflows use it for |
|---|---|
| Custom instructions (`copilot-instructions.md`) | Agent follows them during workflow execution |
| AGENTS.md | Agent reads it to understand the project |
| Custom agent profiles (`.agent.md`) | Workflow can invoke named agents for specific steps |
| MCP servers | Workflow can call MCP tools (e.g., Sentry, Notion) |
| Hooks | Workflow execution can trigger hooks for validation and audit |
| Agent Skills | Workflow can invoke skills (e.g., `/spec-writer`) |

A mature repository with all these layers configured can run sophisticated workflows: receive a spec, plan the implementation, write code, run tests, review security, check coverage, and open a PR — entirely autonomously.

### Current status

As of April 2026, Agentic Workflows are in technical preview. The `.workflow.md` format and trigger syntax are evolving. Check the [GitHub documentation](https://docs.github.com/en/copilot/agentic-workflows) for the current specification before writing your own.

---

In Ch 16, we'll shift to the concerns that keep security teams and legal departments up at night: **AI-first security and governance**. We'll cover risks like sensitive data exposure in prompts and code injection, agent firewalls that control which domains the coding agent can access, testing and releasing custom agents in organizations, and the intellectual property implications of AI-generated code.
