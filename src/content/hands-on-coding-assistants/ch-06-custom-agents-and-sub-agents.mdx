...
In Ch 5, we learned how to write effective prompts — goal, context, constraints, output format. That works well for individual tasks, but it has a scaling problem: you end up writing the same context, the same constraints, and the same tool permissions into every prompt. Your testing prompts always say "don't modify production code." Your planning prompts always say "use read-only tools." Your review prompts always say "check for security, readability, and performance."
...
Custom agents are reusable roles in your harness: each agent file defines a persona with a specific scope — which tools it can use, what model it runs on, how it behaves. Selecting the agent pre-loads all context for that role, so you're not writing it again from scratch.

The serious version of custom agents is not "give the model a fun personality." It is role design. A good agent has a narrow job, a small tool surface, a clear output contract, and a known handoff point. If you cannot describe what the agent is not allowed to do, the role is probably too broad.
...
For this series, we'll focus on **workspace agents** (`.github/agents/`) because they're version-controlled and shared — the whole team benefits from a well-built agent.
...
| `user-invocable` | No | Whether the agent appears in the dropdown (`true` by default). Set `false` for subagent-only agents. |
...
The body is where you define the agent's behavior in natural language. This is your prompt engineering from Ch 5, encoded permanently:
...
This is also a productivity principle. Broad tools increase the number of possible actions the agent can take, which increases review burden. A planner with write access will eventually write. A reviewer with edit access will eventually "helpfully" patch code. Tool scope protects both security and workflow clarity.
...
Before you add an agent to the repository, review it like production code:

| Question | Why it matters |
|---|---|
| What task does this agent own? | Prevents overlapping roles and confusing handoffs. |
| What tools does it actually need? | Enforces least privilege and reduces accidental mutation. |
| What output format does it produce? | Makes results comparable and reviewable. |
| What evidence must it provide? | Forces tests, commands, file references, or explicit assumptions. |
| When should it stop and ask? | Prevents silent assumptions on ambiguous or risky work. |

If an agent cannot pass this review, keep using a prompt until the repeated pattern is clearer.

---

## Building your first agents

Let's build four agents that cover the most common workflows: planning, implementing, testing, and reviewing.
...
### The context cost of multi-agent systems

Subagents isolate context. But they don't eliminate the token cost of what they return. Understanding that cost helps you design workflows that don't defeat their own purpose.

**The scale of the problem:** multi-agent systems use approximately 15× more tokens than regular chat interactions (Anthropic Engineering). That's not a reason to avoid them — it's a reason to design them intentionally.

**Subagents return summaries, not raw output.** This is the core of the token savings. The subagent might read 40 files, run 15 searches, and explore three dead ends — none of that appears in the main agent's context. Only the final result comes back. If you ask a subagent to "analyze the authentication system for security issues," the main agent receives "Found 3 issues: [concise list]", not the full exploration transcript.

**But anti-patterns still cause context explosion:**
- Spawning many subagents that each return long, detailed results → the main agent's context fills up anyway
- Using subagents for tasks that need tight back-and-forth with the main agent → overhead exceeds benefit
- Deep nesting without a purpose — VS Code enforces a maximum depth of 5 levels (`chat.subagents.allowInvocationsFromSubagents` must be enabled for any nesting at all)

**When to use inline vs. subagent — a decision table:**

| Use the main conversation | Use a subagent |
|---|---|
| Needs frequent back-and-forth | Produces verbose output you don't need in main context |
| Multiple phases share significant context | Work is self-contained with a clear deliverable |
| Quick, targeted change | Needs restricted tools or a different model |
| Latency matters (subagents rebuild context from scratch) | Parallel independent investigation |

### Model routing: cheaper models for worker agents

Custom agents can specify a model via the `model` frontmatter field. This matters for cost. When a coordinator delegates to a worker agent, the worker doesn't need the most powerful (and expensive) model — it has a focused, narrow task.

```markdown title=".github/agents/test-runner.agent.md"
---
name: Test Runner
...
tools: ['terminal', 'read']
---
Run the test suite and return only: total tests, passing, failing,
and the full output for any failing tests.
```

The coordinator uses the full model for reasoning and synthesis. Worker agents use faster, cheaper models for execution. This is the same principle as using spot instances for parallelizable batch jobs: match the cost tier to the task complexity.

VS Code enforces one constraint: a sub-agent's model cannot exceed the cost tier of the main model. You can route down (cheaper) but not up (more expensive) within a session.
...
Do not let a coordinator hide risk. A coordinator that delegates to several workers still needs to return a concise final report: what changed, which checks ran, which acceptance criteria are satisfied, and what remains uncertain. Otherwise the workflow only moved complexity from your screen into invisible subagent transcripts.
...
```mermaid
flowchart LR
  P["🔍 Planner"] -->|"Start Implementation"| I["⚡ Implementer"]
  I -->|"Run Tests"| T["🧪 Test Specialist"]
  I -->|"Request Review"| R["👀 Reviewer"]
  T -->|"Request Review"| R
```
...
We already used this same pattern in the planner and implementer agents earlier in this chapter. The key configuration is the `handoffs` array: each entry defines a `label` (button text), the target `agent`, the `prompt` (pre-filled context for the next agent), and whether to `send` automatically or let the user review first. This creates a guided sequential workflow — the system suggests the next step, but you decide whether to take it.

---

## Subagent-only agents

Not every agent needs to appear in the dropdown. Some agents exist purely as workers for other agents. Use `user-invocable: false` to create hidden agents:
...
Use the same project from Ch 5, or create a new one:
...
| **Subagent-only agents in dropdown** | Cluttered agent picker with agents that shouldn't be used directly | Set `user-invocable: false` for worker-only agents |
...
In Ch 7, we'll shift focus to **`AGENTS.md` and project context** — the file that teaches _all_ agents (including the coding agent on GitHub) how your project works, its build steps, conventions, and architecture.
