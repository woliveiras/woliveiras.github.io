...
Copilot Code Review will _not_ catch most of these. It reviews the code as written, not as executed. This is why **running the code** and **running the tests** are non-negotiable steps in your Review phase. Tests catch most hallucinations; a thorough manual read catches the rest.
...
In Ch 12, we'll shift focus to the **Correct** phase — debugging, refactoring legacy code, and the iteration strategies that make the difference between an engineer who fights the AI and one who collaborates with it effectively.
