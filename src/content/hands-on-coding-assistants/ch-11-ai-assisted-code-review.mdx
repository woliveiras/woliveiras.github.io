...
In the previous chapter, we used AI to generate tests and then reviewed those tests by running the suite. The review was essentially binary: pass or fail. Code review is different. It demands nuanced judgment: is this naming clear? Is this pattern consistent with the codebase? Could this query be exploited? For decades, that kind of judgment required a human reviewer. Now, AI can provide a meaningful first pass, catching issues before your teammates even open the pull request.

This chapter focuses on **Copilot Code Review**: how it works, what it catches (and misses), how to customize its behavior with the instruction files you built in Module 1, and how to configure automatic reviews so every PR gets AI feedback without anyone lifting a finger. We'll also cover the **"Implement suggestion"** feature, where Copilot's coding agent takes review comments and turns them into actual code changes — closing the review loop all the way back to implementation.

The goal is not to make AI the reviewer of record. The goal is to reduce low-value review noise before a human spends attention on the PR. AI review is useful when it is specific, diff-local, and actionable. Generic comments are worse than silence because they train the team to ignore the reviewer.
...
### Weak vs. actionable AI review comments

Empirical work on AI code review comments in GitHub Actions found that concise, chunk-level comments with concrete code context are more likely to lead to actual code changes. That matches normal human review practice: a reviewer who points to the exact line and explains the impact is more useful than a reviewer who leaves a broad opinion.

Use this standard for AI review instructions:

| Weak comment | Actionable comment |
|---|---|
| "Consider improving error handling." | "`src/api/users.ts:42` returns the raw database error to the client. Map it to `AppError` so stack traces and constraint names are not exposed." |
| "This may be insecure." | "`query = 'SELECT * FROM users WHERE email = ' + email` builds SQL by concatenation. Use the existing parameterized query helper from `src/db/query.ts`." |
| "Add tests." | "Add an error-path test for duplicate email registration. The current tests cover 201 and 400, but not the 409 path introduced in this PR." |
| "Maybe simplify this function." | "`calculateInvoiceTotal` now handles discounts, taxes, and currency formatting. Extract formatting into `formatCurrencyAmount` so the calculation path stays testable." |

When you configure a review agent, require three fields for every finding:

1. **Location**: file and line or diff chunk.
2. **Impact**: what can break, leak, regress, or confuse.
3. **Fix**: a concrete change, not a vague improvement.

### Scoped instructions with `.instructions.md`

Remember from Chapter 8 that `.instructions.md` files apply to files matching their `applyTo` glob patterns. This is powerful for code review:
...
This closes the loop: Copilot reviews the code → you decide the suggestion is valid → Copilot implements the fix in a new branch → you review the resulting PR → merge.
...
### Hallucination in generated code: what to look for

Before reviewing the review itself, it helps to know the specific failure modes of AI-generated code. These are distinct from normal bugs — they're patterns where the model produces plausible-looking code that is quietly wrong.

| Hallucination type | Example | How to catch it |
|---|---|---|
| **Invented API methods** | `array.findAndRemove()` — doesn't exist in JavaScript | Run the code; TypeScript will also flag it |
| **Wrong library version syntax** | Using React 17 class component syntax in a React 19 project | Check the package.json version, then the docs |
| **Non-existent config options** | `{ retries: 3, backoffStrategy: "exponential" }` — option names invented | Read the library's actual documentation |
| **Invented internal APIs** | Calling `this.userService.findByEmailCached()` when that method doesn't exist in your codebase | Grep/search the codebase for the method |
| **Plausible but incorrect SQL** | `WHERE created_at > NOW() - INTERVAL 30 DAY` (MySQL syntax in a PostgreSQL project) | Verify dialect-specific syntax |
| **Wrong import paths** | `import { validate } from '../utils/validation'` when the file is at `'../lib/validation'` | Run the build; TypeScript catches this |

**The key signal for hallucinations:** code that _looks_ right but isn't wired to anything real. The model learned common patterns and reproduced them — but used method names, option keys, or import paths that don't exist in your actual codebase.

Copilot Code Review will _not_ catch most of these. It reviews the code as written, not as executed. This is why **running the code** and **running the tests** are non-negotiable steps in your review. Tests catch most hallucinations; a thorough manual read catches the rest.

---

### The three-pass review structure

Reviewing AI-generated code effectively requires a different structure than reviewing human-written code. Human-written code arrived after the author thought it through; AI-generated code arrived after a model predicted the most statistically likely token sequence. The review needs to compensate for that difference.

A practical structure is three sequential passes:

**Pass 1 — Automated (CI)**

Let your pipeline do the mechanical verification. This pass doesn't require a human:

- [ ] Build succeeds
- [ ] Existing tests pass
- [ ] New tests pass
- [ ] Lint/formatting passes
- [ ] Coverage hasn't dropped (if you have a ratchet configured — Ch 10)

If any of these fail, the PR isn't ready for human review. Automation catches the obvious failures so humans can focus on what automation can't catch.

**Pass 2 — Spec compliance**

Compare the PR against the acceptance criteria from the spec. This pass answers: _did the agent do what it was asked to do?_

For each acceptance criterion:
- [ ] Was it implemented?
- [ ] Is it implemented correctly (not just "something that looks like it")?
- [ ] Did the agent add anything that's explicitly Out of Scope?

This pass should be fast when the spec is well-written: you're checking a list of binary conditions. When it's slow, the spec was probably vague.

**Pass 3 — Quality and judgment**

This is the human review. It answers questions that neither automation nor spec compliance can answer:

- [ ] Would I write this code this way? If not, why not?
- [ ] Are there any hallucinations (invented APIs, wrong syntax, non-existent imports)?
- [ ] Does this code fit the codebase? Does it follow the patterns in adjacent files?
- [ ] Would a teammate understand this in six months?
- [ ] Are there any security concerns that automated scanning might have missed?
- [ ] Are the tests testing behavior, or are they testing the implementation?

**Why three passes?** Each pass has a different cost structure. Pass 1 is free (automated). Pass 2 is fast (binary checklist). Pass 3 is where you spend your judgment. Running them in order means you only reach the expensive pass when the cheap passes have already passed — avoiding the situation where a human reviewer spends 20 minutes reviewing code that doesn't even build.
...
4. **Don't skip reviewing the reviewer.** Copilot's review comments are AI output. Apply the same scrutiny you give to generated code: validate the comment before implementing it, and correct any that are wrong.
...
| Step | What happened |
|---|---|
| **Setup** | You wrote custom instructions defining what "good code" means for this repo |
| **Automation** | You configured Copilot to automatically review every PR |
| **Review** | You triaged Copilot's comments — accepting, implementing, or dismissing each one |
| **Harness improvement** | You refined your instructions based on false positives and gaps |
...
Copilot Code Review brings automated, spec-aware review to pull requests at scale. Let's recap the key points:
...
In Ch 12, we'll shift focus to debugging, refactoring legacy code, and the iteration strategies that make the difference between an engineer who fights the AI and one who collaborates with it effectively.
