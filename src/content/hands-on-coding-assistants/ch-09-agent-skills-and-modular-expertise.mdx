...
**Agent Skills solve this.** A skill is a folder of instructions, scripts, and resources that the agent discovers at startup but only loads when the task calls for it. You can think of skills as plug-in expertise: modular, conditionally-activated, and reusable across tools. They're the expertise layer of the harness — specialized knowledge that loads on-demand without bloating every interaction.

Skills let you teach the agent a procedure once — how to write a spec, how to debug a CI workflow, how to set up a new project — and let it pull that knowledge in whenever it recognizes the pattern. Less repetition, more reliable results.

The best skills are boring in the right way. They capture a workflow that already works, including the commands, failure modes, and review criteria. A skill should not invent a process; it should make a proven process repeatable.
...
| `user-invocable` | Controls whether the skill appears in the `/` slash command menu. Default: `true` | `false` (hides from menu but still auto-activates) |
...
| `context` | Execution context: `inline` (default) or `fork` | `fork` |

### `context: fork` — running a skill in its own context

By default, skill instructions are injected inline into the current agent's context window (mode `inline`). This is fine for short skills that provide a few rules or commands.

For skills that read many files, run lengthy investigations, or produce a focused deliverable (a report, a diff, a test summary), use `context: fork`:

```yaml
---
name: security-audit
description: >
  Performs a security audit of the codebase — checks for OWASP Top 10 vulnerabilities,
  exposed secrets, and insecure dependencies. Use when asked to audit security,
  review code for vulnerabilities, or before a release.
context: fork
---
```

With `context: fork`, the skill executes in a **dedicated sub-agent**. All its file reads, tool calls, and intermediate reasoning load into the sub-agent's context — not yours. The parent agent receives only the skill's final output.

This mirrors exactly what custom agents do when a coordinator delegates to a worker sub-agent (Ch 6). The difference is you get the same context isolation automatically, just by setting a frontmatter field — no agent orchestration required.

**When to use `fork`:**
- The skill reads a large number of files (code audit, dependency analysis, full codebase search)
- The skill produces one focused artifact (a report, a structured list, a refactoring diff)
- You don't want the skill's exploration to pollute the main conversation

**When to keep `inline` (default):**
- The skill provides a few targeted rules the agent needs to know mid-task
- The skill output feeds directly into the current turn's reasoning
- The skill is short and narrow

`context: fork` requires the `github.copilot.chat.skillTool.enabled` VS Code setting. It is documented as experimental, so use it selectively and verify support in your team's VS Code and Copilot versions before relying on it for critical workflows.

### Invocation control matrix

The combination of `user-invocable` and `disable-model-invocation` gives you four modes:

| `user-invocable` | `disable-model-invocation` | `/` menu | Auto-activate | Best for |
|---|---|---|---|---|
...
Before a skill becomes part of a team harness, check it against this bar:

| Dimension | Requirement |
|---|---|
| Trigger clarity | The description says exactly when to use the skill and includes the words users actually say. |
| Procedure | The body gives ordered steps, not vague principles. |
| Verification | The skill states which command, output, or artifact proves the task is complete. |
| Failure handling | Known errors and recovery steps are documented. |
| Scope | The skill says what not to touch. |
| Security | Any credential, network, filesystem, or production action requires explicit approval. |

This bar matters because skills are portable. A weak skill does not stay local; it spreads across projects and repeats the same mistake at higher speed.
...
| Background knowledge only relevant sometimes | `user-invocable: false` — agent loads it automatically, no clutter in the `/` menu |
...
With Ch 3 through Ch 9, you've now seen every customization mechanism. Here's the full picture:
...
## Hands-on: build a spec-writer skill

In Ch 2, you practiced writing specs manually: Goal, Acceptance Criteria, Technical Notes, Out of Scope. After a few iterations, the structure becomes second nature. But the blank page problem persists: when someone drops a vague issue description in front of you, there's still friction in transforming it into a proper spec.

A `spec-writer` skill eliminates that friction. You invoke it with a vague description, and the agent produces a structured spec draft — which you then review and refine before delegating the implementation.

This is the first exercise where you're using a skill to automate something you already know how to do manually. That's the right order: understand the process first, then automate it.

### Step 1: create the directory structure

```sh
mkdir -p .github/skills/spec-writer
```

### Step 2: create the SKILL.md

Create `.github/skills/spec-writer/SKILL.md`:

```markdown
---
name: spec-writer
description: >
  Turns a vague feature request, issue description, or idea into a
  structured Spec-Driven Development spec. Use when the user says
  "write a spec", "turn this into a spec", "spec out this feature",
  or provides a rough description of a task to be delegated to an agent.
argument-hint: "[paste vague issue or feature description here]"
---
...
Transform a rough task description into a structured spec ready for
agent delegation. The spec format follows Spec-Driven Development (SDD):
four sections that give the agent everything it needs to implement
correctly without ambiguity.

## Output format

Always produce a spec in this exact structure:

```markdown
## Goal

[1–3 sentences: what this task accomplishes and why it matters.
Be concrete about the user or system benefit.]

## Acceptance criteria

- [ ] [Specific, testable criterion — an observable behavior or outcome]
- [ ] [...]
- [ ] [Tests: describe what test coverage should exist]

## Technical notes

- [Relevant files, patterns, or conventions to follow]
- [Constraints: libraries to use, patterns to avoid, performance targets]
- [Dependencies: APIs, schemas, or services this task touches]

## Out of scope

- [What this task explicitly does NOT include]
- [Future enhancements that are related but separate]
```text

## Instructions

1. **Read the input carefully.** Identify the core action, the affected
   system, and the likely users or callers.

2. **Write the Goal.** One to three sentences. What does this do, and why
   does it matter? Do not describe how to implement it.

3. **Write Acceptance Criteria.** Each criterion must be:
   - Specific (not "it works" — describe the exact behavior)
   - Testable (a pass/fail question, not a quality judgment)
   - Scoped (not broader than the stated goal)
   Include at least one criterion for tests or verification.

4. **Write Technical Notes.** Ask: what does an agent need to know to
   stay consistent with this codebase? Reference files, patterns, or
   constraints from the input. If none are mentioned, note what would
   typically apply (e.g., "follow existing route patterns").

5. **Write Out of Scope.** Identify at least one thing that is clearly
   related but should NOT be part of this task. This prevents scope
   creep on the agent's part.

6. **Flag ambiguities.** After the spec, add a brief "Open questions"
   section if the input is missing information critical to the spec.
   For example: "Open questions: Which API endpoint handles user lookup?
   Is partial-match search expected?"

## Quality bar

A good spec should be precise enough that someone unfamiliar with
the codebase could implement it correctly, and complete enough that
the acceptance criteria function as a review checklist.

Do not include implementation details (how to do it — that's the
agent's job). Do include behavioral contracts (what it does — that's
your job as the author of the spec).
```

### Step 3: test the skill

Create a test issue description and invoke the skill directly:

```text
/spec-writer

Users should be able to upload a profile picture. It should be
resized automatically and stored somewhere. Update their profile
when done.
```

Evaluate the output against the Ch 2 quality bar:
- Is the Goal one to three sentences, describing the what and why?
- Are the acceptance criteria specific and testable (not "image uploads work")?
- Does Technical Notes ask the right questions even when the input didn't provide context?
- Is there at least one meaningful Out of Scope item?
- Were ambiguities flagged? (The example above has several: max file size? supported formats? storage service?)

**Iterate on the SKILL.md** until the output consistently passes this bar. A few common adjustments:

| Problem in output | Fix in SKILL.md |
|---|---|
| Criteria too vague ("upload works") | Strengthen step 3 with examples of what "specific" means |
| No test criterion generated | Add an explicit rule: "Always include at least one criterion specifying what tests should exist" |
| Technical Notes left empty | Add: "If no technical context is given, list what you'd need to know to write the spec" |
| Out of scope is empty | Add: "Always include at least one item, even if you have to infer what's adjacent" |

### Step 4: integrate with your delegation workflow

Once the skill produces reliable specs, your delegation flow becomes:

1. Receive vague issue or feature request
2. Run `/spec-writer [description]` — agent produces a draft
3. Review and refine the draft (you own the spec, the agent drafted it)
4. Use the final spec as the issue body when assigning to the coding agent
5. After the PR is created, score it against the acceptance criteria

This is Ch 2's Spec-Driven Development cycle, accelerated. You still review and own every spec — the skill handles the blank page.
...
After using the skill a few times, you'll notice gaps. Maybe you need a section on matrix builds, or caching strategies, or specific workflow patterns your team uses. Update the `SKILL.md`, add more reference files, and commit. Skills are living documents — the same principle we applied to AGENTS.md in Ch 7.
...
In Ch 10, we shift from customization to application. We'll take everything we've built in Module 1 and put it to work on **test generation** — the highest-impact use case for AI coding assistants, where the delegation and review patterns we've established really shine.

> **If you want a ready-made harness:** [Geremmyas](https://woliveiras.com.br/posts/introducing-geremmyas-spec-driven-development-ai-assistants/) ships a production version of everything we built in Modules 0 and 1 — AGENTS.md, instructions, a `spec-writer` skill, guardrail hooks, and reviewer agent profile — as a single install command. Use it to bootstrap any new project with the full spec-first workflow already in place.
