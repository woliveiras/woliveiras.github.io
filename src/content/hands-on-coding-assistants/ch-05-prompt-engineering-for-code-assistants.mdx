...
## Your prompt derives from your spec

In Ch 2, we introduced the spec-first workflow and the four-section spec: Goal, Acceptance Criteria, Technical Notes, Out of Scope. The spec defines what "done" looks like before you open any AI tool.

Your prompt is how you hand that spec to the AI tool. Notice the parallel: Goal maps to the goal component of your prompt, Acceptance Criteria and Technical Notes map to constraints and context, and Out of Scope maps to boundaries. If you've written a good spec, writing the prompt becomes straightforward — you're translating a structured document into a conversation, not inventing context from scratch.

A vague prompt is a symptom that the spec was skipped. Without clear acceptance criteria, you'll spend more rounds correcting misalignments, wondering why the AI "didn't understand."

This chapter is about the craft of that translation — how to turn a solid spec into prompts that work on the first try, not the fifth.

The mistake I see most often is treating prompt engineering as wordsmithing. It is not. For coding assistants, prompt quality mostly comes from engineering quality: clear contracts, visible context, small scope, and an objective way to check the answer.
...
GitHub's documentation, VS Code's prompt crafting guide, and research from teams using Copilot at scale converge on five strategies that consistently improve output quality. These are the patterns that consistently reduce your review and correction cycles.
...
Each step is a single-responsibility delegation. The model does one thing well, you review it, and the next step builds on verified code. Plan each step, write it as a prompt, review the output, correct before moving on.
...
### Managing context over a session

A single prompt is one write–review cycle. A long agent session is many cycles, and each cycle consumes context window tokens. As a session grows, two things happen:

1. **Earlier context gets compressed or evicted.** The model has a fixed context window. When it fills, older content is summarized or dropped. Instructions you gave at the start of the session may no longer have full weight.
2. **Accumulated wrong assumptions propagate.** If the agent made a wrong assumption in iteration 3, it may carry that assumption silently through iterations 4, 5, and 6, building on a broken foundation.

The signals that a session has run too long:

| Signal | What it means |
|---|---|
| Agent contradicts a decision it made 10 turns ago | Earlier context is degraded |
| Responses become generic, not codebase-specific | Session context overwhelmed by token pressure |
| Agent starts "reinventing" patterns it already wrote correctly | Context loss |
| `/context` in the CLI shows 80%+ token usage | Time to compact or restart |

**Practical habits:**

- **Use `/compact`** in the CLI proactively, before you hit the limit. Compaction summarizes the conversation history, preserving the outcomes without the full turn-by-turn context.
- **Start a new thread for unrelated tasks.** Don't ask the implementation agent to "also fix this unrelated bug" in the same session. The unrelated context pollutes the active task.
- **Open a fresh session for large tasks.** When you're starting a major feature, start with a clean session and provide the spec as the first message. Don't inherit stale context from debugging a typo three hours ago.
- **Reference files explicitly, don't rely on session memory.** `#file:src/routes/users.ts` tells the agent exactly what to look at. "The file we discussed earlier" depends on context that may no longer be there.

This isn't a limitation to work around — it's a property to design for. Shorter, well-scoped sessions with explicit context anchors consistently outperform long, sprawling ones.
...
**Review cost:** You'll spend more time spotting issues and fixing them than you saved by writing a short prompt.
...
**Review cost:** One round of review, maybe a small correction for an edge case.
...
**Review cost:** Minimal review, likely zero corrections. The time invested in the prompt pays for itself immediately.
...
Use a different prompt style when your goal is learning. Skill-formation research found that developers can complete AI-assisted tasks while learning less, especially when the assistant hides the reasoning. That is acceptable for a low-risk delivery task you already understand. It is a bad trade for a technology you are trying to master.

In **delivery mode**, ask for implementation, tests, and verification:

```text
Implement the agreed plan. Keep the diff limited to the listed files.
Run the test command and report the result.
```

In **learning mode**, ask for reasoning before code:

```text
Before writing code, explain the design options for this async Python
library. Show the smallest example that demonstrates the pattern, then
quiz me on the failure modes. Do not implement the production change yet.
```

Use learning mode when the domain is new, the failure mode is expensive, or you cannot confidently review the output. The goal is comprehension first, code second.
...
In Ch 6, we'll take the next step: building **custom agents and sub-agents** with `.agent.md` profiles that encode your prompt engineering decisions into reusable, shareable configurations — turning one-off prompts into permanent team knowledge.
