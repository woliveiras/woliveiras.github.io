...
In Ch 4, we made that concrete: you write a spec before delegating. The spec has four sections — Goal, Acceptance Criteria, Technical Notes, Out of Scope — and it answers all five questions in one document.
...
A vague prompt is a symptom that the Plan phase was skipped. The rest of the PDRC cycle inherits that debt: more review rounds, more corrections, more time wondering why the AI "didn't understand."
...
A single prompt is one PDRC cycle. A long agent session is many cycles, and each cycle consumes context window tokens. As a session grows, two things happen:
