...
The harness is also how you avoid turning AI adoption into a personality contest. Without a shared harness, each developer invents their own prompt style, their own safety rules, and their own definition of "done." That does not scale. A principal-level AI workflow makes the good path the default path.
...
Research from DORA frames AI as an amplifier of existing delivery strengths and weaknesses. The harness is how you make sure there is something worth amplifying: reliable tests, explicit conventions, scoped permissions, repeatable review, and measurable outcomes.
...
A harness that nobody maintains becomes stale documentation with executable consequences. Review it on a schedule, and also review it after every serious agent failure.

Use this loop:

1. **Capture the failure**: what did the agent misunderstand, skip, or overreach?
2. **Classify it**: missing project context, missing path-specific rule, missing skill, missing hook, or weak spec?
3. **Patch the harness**: update the smallest layer that would have prevented the failure.
4. **Add a verification command**: make the corrected behavior observable.
5. **Remove dead rules**: stale instructions waste context and create conflicts.

The best instruction file is not the longest one. It is the one that prevents the next expensive mistake.
