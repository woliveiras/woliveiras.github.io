...
Research backs this up. A 2025 study from UC San Diego and Cornell ("Professional Software Developers Don't Vibe, They Control") found that experienced developers treat agents exactly like junior developers: they give clear instructions, stay in the loop, and verify the output. They plan before they delegate. They review before they accept. The developers who got bad results were the ones who handed off without structure and trusted the output without verification.
...
The McKinsey data is direct: AI-assisted code was marginally higher quality only when developers **actively iterated** with the tools. When they just accepted — quality was _worse_ than non-AI code.
...
|---|---|---|
| **Low** | Lightweight — well-formed prompt, quick scan of output | Generating a test for a pure utility function |
| **Medium** | Standard — explicit spec, review the diff, one round of correction | Adding a new endpoint, refactoring a module |
| **High** | Full workflow — written spec, Plan mode review, thorough acceptance criteria check | Security-critical code, authentication, payment logic, database migrations |

The workflow is a spectrum. Use your judgment — but _have_ a structure for how you engage with AI. The alternative is "accept all," and we've seen where that leads.
