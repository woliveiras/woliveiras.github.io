...
Write commands exactly as a teammate would run them. Avoid "run the tests" if the real command is `pnpm vitest run --coverage` or `make test-unit`. Ambiguous commands create agent exploration, and exploration creates wasted time and tokens.
...
Add a short "Definition of done" section to every serious `AGENTS.md`:
...
- Run the full project verification before final handoff when practical.
- Report exact failing commands instead of claiming success.
- Never hide skipped tests, failing lint, or type errors.
```

This section prevents a common failure mode: the assistant says "done" because it edited files, while the repository cannot build. Editing is not completion. Evidence is completion.
...
## Auto-generating Copilot instructions, then adapting to AGENTS.md

You don't have to write AGENTS.md from scratch. GitHub provides an official prompt that instructs the Copilot coding agent to analyze your repository and generate a comprehensive Copilot instructions file. This is the recommended starting point for existing projects, and it gives you most of the operational content you need for AGENTS.md.
...
- Your project has documentation that already exists in `docs/` (or `CONTRIBUTING.md`, `SECURITY.md`, etc.)
- Different tasks need different subsets of that documentation
...
- "This project uses `Result<T, E>` return types instead of throwing exceptions"
- "The developer always prefers early returns over nested conditionals"
- "Tests in this project are organized by feature, not by test type"
- "The `UserService` is the authoritative source for user-related operations; don't add user logic elsewhere"
...
This means Memory doesn't solve the context window management problem from Ch 5. If your session grows too large, Memory doesn't extend the window. But when you start a _new_ session, Memory pre-populates the agent with relevant repository knowledge, so you don't have to re-establish context from scratch every time.
...
Create a custom agent when you want a **persistent, named persona** that the team can select from the dropdown. The agent defines _who the AI is for this class of task_ — and it stays consistent across every invocation:
...
```mermaid
flowchart TD
    A[New task or need] --> B{Does the team repeat this<br/>workflow regularly?}
    B -- No --> C[Write a clear prompt.<br/>No artifact needed.]
    B -- Yes --> D{Is this a role with<br/>tool/model constraints?}
    D -- Yes --> E["Create a custom agent<br/>(.agent.md)"]
    D -- No --> F{Is this a domain procedure<br/>portable across tools?}
    F -- Yes --> G["Create a skill<br/>(SKILL.md)"]
    F -- No --> H{Does it need<br/>to run as a sub-agent<br/>in an orchestration?}
    H -- Yes --> E
    H -- No --> C
```
...
- For one-off parallelism (e.g., `/fleet` in Copilot CLI), no agent files are required — the built-in behavior handles it
- Agent files add value when the configuration **needs to be shared, versioned, and consistent across team members**, not for private one-off tasks
...
7. **Use AGENTS.md as a documentation index.** Keep AGENTS.md thin. For larger projects, replace inline content with a table linking to `docs/ARCHITECTURE.md`, `docs/SECURITY.md`, `docs/GLOSSARY.md`, etc. Include a "Read when..." column so the agent knows which documents are relevant to which tasks.
8. **Copilot Memory complements AGENTS.md.** Use AGENTS.md for deliberate, curated project context. Let Memory capture the patterns and corrections that emerge from real use.
9. **Default to doing nothing.** Copilot auto-spawns sub-agents and handles most tasks with a good prompt and AGENTS.md context. Create a custom agent when a persona needs to be shared and versioned across the team. Create a skill when you have a portable, domain-specific procedure. If only you benefit, use a prompt — not a file.
