...
## When the AI gets it wrong: debugging, refactoring, and iteration

Previous chapters covered generating tests and reviewing code. But what happens when the AI gets it wrong? When the generated code doesn't work, when the refactored module breaks in production, when the agent goes in circles? This is where **engineering judgment** — the distinctly human part of the workflow — is non-negotiable.

This chapter covers three things that all require that judgment: **debugging** (finding and fixing what's broken), **refactoring** (restructuring what works but shouldn't stay as-is), and **iteration** (the strategies for steering the agent back on track when it drifts). We'll also cover **codebase onboarding** (using AI to quickly understand unfamiliar projects).

The research is useful here because it is mixed. Peng et al. found large speedups on a bounded coding task. Cui et al. found enterprise productivity gains across field experiments. METR found experienced maintainers were slower with AI on mature open-source tasks. The difference is not whether the tool is "good." The difference is whether the task is bounded, the context is visible, and the developer can steer correction without creating a bigger mess.

That is why this chapter is mostly about iteration discipline. The developers who capture value are not the ones who accept the first generated patch. They are the ones who can add missing context, reduce scope, create a safety net, and recognize the moment manual intervention is cheaper than another prompt.
...
Debugging with AI follows a tight cycle:
...
If debugging is about fixing what's broken, refactoring is about improving what works but is hard to maintain, extend, or understand. AI is useful here because many refactorings are pattern-heavy. The risk is that the agent changes behavior while making the code look cleaner.

Treat refactoring as a preservation task. The behavior is the contract. The diff is only acceptable if the safety net proves the contract still holds.
...
As with test generation (Chapter 10), AI accuracy decreases as scope increases. Single-function refactoring is nearly always correct. Full-codebase restructuring requires human architectural judgment with AI as a tool, not a driver.
...
Before refactoring anything, make sure you have tests. If the module has no tests, use the techniques from Chapter 10 to generate them. Your tests are the "before" snapshot — they tell you if the refactoring broke anything.
...
If any test fails, iterate: figure out what the refactoring broke, provide that context to the AI, and correct.
...
Codebase onboarding is a good use of AI, but only if you treat the answer as a map to verify. Rasnayaka et al. found that LLMs helped with early development, syntax, and debugging, while integration still required human work. Skill-formation research adds the caution: if the assistant gives you the answer without making you build the mental model, you may move faster today and understand less tomorrow.

Use AI onboarding to create paths through the codebase, not to replace reading the code.
...
If the repository has an `AGENTS.md` or `copilot-instructions.md` file (Chapters 7 and 8), the agent already has curated context about the project's architecture, conventions, and patterns.
...
## Iteration strategies: steering the agent when it drifts

### When the agent goes off-rails

Every developer who uses agents long enough encounters the same scenario: you delegated a scoped task, and the agent went beyond the scope — touching files it wasn't supposed to touch, adding abstractions that weren't requested, refactoring code that was working fine, or installing packages you didn't ask for.

This is one of the most common failure modes in agent-oriented development. It's not a bug — it's the agent optimizing for "completeness" without understanding that completeness isn't the goal. Scope compliance is.

**Early warning signs (check these before accepting a large diff):**

| Signal | What it means |
|---|---|
| Files appear in the diff that aren't in the spec's Technical Notes | Agent went out of scope |
| New packages appear in `package.json` / `requirements.txt` | Agent self-installed dependencies |
| Functions or classes are renamed across the codebase | Agent decided to "clean up" while it was in there |
| New abstractions appear that aren't referenced by the acceptance criteria | Agent added "future flexibility" you didn't ask for |
| The diff is 3x larger than you expected for the task | Strong signal of scope creep |

**How to interrupt and recover:**

If you're watching the agent work in real time (VS Code Agent mode or CLI):

1. **Stop the agent** before it commits or stages changes. Click the stop button or press `Ctrl+C`.
2. **Discard the off-scope changes.** Use `git checkout -- <file>` for specific files, or `git checkout .` to reset the entire working tree.
3. **Re-scope the spec.** Add explicit Out of Scope items that name the patterns you saw: "Do not rename any existing functions," "Do not add new dependencies," "Only modify files in src/routes/."
4. **Re-delegate.** Start a fresh session with the tightened spec.

If you only catch scope creep at the PR stage (coding agent on GitHub):

1. **Close the PR without merging.** Don't try to patch it with review comments.
2. **Strengthen the spec and re-assign.** A PR with scope creep is a signal the spec's boundaries were unclear, not a PR worth salvaging line by line.

The Out of Scope section of your spec (Ch 2) is your primary defense. The minimal footprint instruction in `copilot-instructions.md` (Ch 8) is your secondary defense. Use both.

---

### Agent session boundaries: when to restart

A long agent session accumulates context: previous answers, corrections, files it read, commands it ran. This is useful up to a point. After that point, it becomes a liability.

**When a session has gone stale:**

- The agent starts contradicting decisions it made earlier in the same session
- Responses become less specific to your codebase and more generic
- The agent rewrites something it already wrote correctly, without a reason
- `/context` in the CLI shows 80%+ token usage
- You're asking about Feature B but the agent keeps referencing context from Feature A

**What to do:**

1. **Run `/compact`** (CLI) to summarize history. This preserves the outcomes while reducing the token footprint. Good for mid-task compaction on large features.

2. **Write a handoff summary** (see next section) and start a fresh session. Good when the task shifts significantly or when you're resuming after a break.

3. **Start completely fresh** for unrelated tasks. Context from the previous task actively hurts the new one.

**The discipline:** treat sessions like pull requests. Each session should have a clear scope, a defined start, and a defined end. Sessions that sprawl across multiple unrelated tasks produce worse results than focused sessions, for the same reason that PRs that mix multiple changes are harder to review.

This is the small-batch principle applied to agent sessions. DORA's AI research warns that AI adoption can increase batch size and review burden. A long session that edits twenty files is just a large PR with a chat transcript attached. Keep the session small enough that you can still reason about it.

---

### The handoff summary

When work spans multiple sessions — or when you want to hand off to the asynchronous coding agent with full context — write a **handoff summary** before starting the new session.

A handoff summary is a short document (not a long spec) that answers:

```markdown
## Handoff: [task name]

### What's done
- [Completed sub-tasks, with the key decisions made]

### What's next
- [The next sub-task or the remaining acceptance criteria]

### Key decisions made
- [Any architectural or implementation decisions the next session should respect]
- [Files that were intentionally not touched and why]

### Files modified
- [List of files changed, so the next session knows where the work happened]

### Open issues
- [Anything unresolved that the next session should know about]
```

Use this summary as the **first message** of the new session, before any other context. This ensures the agent starts with the facts you care about, rather than building its own model from scratch by reading the entire repository.

For the coding agent on GitHub: paste the handoff summary into the issue body (or a comment) when you re-assign. The agent reads the issue as its primary instruction.

---

The difference between a productive AI collaboration and a frustrating one almost always comes down to iteration skill: knowing how to steer the agent when it goes wrong.
...
After the manual fix, you can re-engage the AI for the next task. Not every step needs to use AI — using the right tool for each sub-task is the whole point.
...
**Corrections are investments.** Every correction that turns into a custom instruction prevents the same mistake for every future interaction. Iteration cost drops over time as your instructions mature.
...
The exact numbers will differ by team, but the pattern is the one you want to measure: correction effort should shrink as your instructions mature, your prompting skill grows, and the AI's context about your project becomes more accurate. If correction effort does not shrink, the workflow is not learning from its own failures.
...
| Step | What happened |
|---|---|
| **Assessment** | You assessed the legacy module and decided on a three-pass refactoring strategy |
| **Delegation** | You gave the agent focused, single-pass refactoring prompts |
| **Review** | You ran the test suite after each pass to catch regressions |
| **Correction** | You identified what the agent got wrong, re-prompted with specifics, and documented recurring issues as custom instructions |
...
Iteration and correction are where AI assistance either pays off or collapses. Without effective correction, AI assistance is a coin flip. With a disciplined correction practice, AI becomes a reliable multiplier:
...
5. **Corrections are investments.** Every correction that turns into a custom instruction prevents the same mistake for every future interaction. Iteration cost drops over time as your instructions mature.

In Ch 13, we'll shift from correcting code to **documenting** it — AI-assisted code documentation, API docs, commit messages, and PR descriptions. Documentation is high-value, pattern-heavy work that AI handles well and humans review quickly.
