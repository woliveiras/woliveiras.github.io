...
Module 3 opens the walls. Starting with this chapter on MCP, then continuing with hooks and automation in Ch 15, you'll learn how to connect Copilot to the external systems that surround your code, and how to orchestrate fully automated workflows that go from issue to pull request without manual intervention.

Opening the walls changes the risk model. MCP tools are not just context. They are privileged integrations into issue trackers, observability systems, cloud accounts, databases, browsers, and internal documentation. Treat an MCP server as part of your software supply chain.
...
### How MCP fits into the spec-first workflow

MCP extends what the agent can access at every stage:

- **Before implementation**: the agent can query Sentry for recent errors, read Notion design docs, or check Azure resource states before planning its approach.
- **During implementation**: prompts can include live data from external systems, making delegations far more precise ("fix the error from Sentry issue PROJ-1234" becomes actionable because the agent can fetch the full stack trace).
- **During review**: the agent can verify its work against external systems (did the fix resolve the Sentry error? does the PR description match the Notion spec?).
- **Feedback loop**: results from external tools feed directly into subsequent corrections.
...
That is useful, but it also means the agent can retrieve poisoned context. Issues, docs, comments, API responses, and tool metadata can all contain prompt injection. The safe MCP posture is: retrieve narrowly, prefer read-only tools, inspect tool output before acting, and keep write-capable tools behind explicit approval.
...
Let's walk through four real-world MCP integrations, from bug tracking to infrastructure. Each example shows the configuration JSON, explains which tools are enabled and why, and illustrates a concrete use case.
...
- The agent reads the issue, then calls `get_issue_details("PROJ-4521")` to fetch the full stack trace from Sentry. It identifies the error: a `NullPointerException` in `PaymentService.processOrder()` where `order.getShippingAddress()` returns null.
- The agent generates a fix — adding a null check and a descriptive error message before the address is accessed.
- You review the PR. The stack trace is referenced in the PR description. The fix matches the exact code path from Sentry.
- If the fix is incomplete, you comment on the PR and the agent iterates.
...
Azure MCP uses the Azure Developer CLI for authentication. You'll need to run `azd coding-agent config` locally to set up the authentication flow and create a `copilot-setup-steps.yml` file (we'll cover this file in detail in Ch 15).
...
4. **Use it** — the agent now has tools it can call autonomously to gather context and take actions.

Add a fifth step for professional use: **audit it**. Record which tools were enabled, which token scopes they received, who approved the integration, and how to disable it quickly. The official [MCP security best practices](https://modelcontextprotocol.io/docs/tutorials/security/security_best_practices) emphasize consent, least privilege, scoped authorization, and confused-deputy mitigations for exactly this reason.
...
Every MCP tool call has a cost: tokens. Understanding that cost is the difference between a configuration that makes the agent smarter and one that makes it slower, more expensive, and less accurate.

### The three primitives — and how each hits your context budget

MCP exposes three types of data to the agent. Each enters the context window differently:

| Primitive | Who controls it | When content enters context | Cost structure |
|---|---|---|---|
| **Tools** | Model-controlled | (1) Schema always upfront; (2) result after each call | Double cost: schema on every request + result per invocation |
| **Resources** | Application-driven | Only when explicitly attached | Opt-in — zero cost until you add it |
| **Prompts** | User-controlled | When user triggers them (slash command) | One-time when triggered |

The tool schema cost is the biggest surprise. When you enable an MCP server with 20 tools, all 20 tool names, descriptions, and input schemas are injected into the system prompt **on every request** — before the agent writes a single line of code.

### The 128-tool limit and the 150,000-token problem

VS Code enforces a hard limit of **128 tools per request**. If you exceed it, you see an error.

But the more insidious problem is token cost. Research from the MCP Client Best Practices documentation quantifies it:

> "Loading all tools upfront consumes ~150,000 tokens on definitions alone, while progressive discovery uses ~2,000 tokens by loading only what the task requires."

At typical pricing, 150,000 tokens in definitions on every request — regardless of task — is expensive. For most coding sessions, 140,000 of those tokens are irrelevant noise.

Large tool surfaces also degrade judgment. The agent has more possible actions to choose from, more schemas competing for attention, and more ways to solve the wrong problem. Progressive discovery is both a cost practice and a reliability practice.

### What to do: progressive discovery

When you have many MCP servers active, the solution is **progressive discovery**: expose a lightweight `search_tools` meta-tool that returns name + one-line description, and load full schemas only for the tools the current task actually needs.

The VS Code setting that automates this:

```json
// .vscode/settings.json
{
  "github.copilot.chat.virtualTools.threshold": 20
}
```

When the number of enabled tools exceeds this threshold, VS Code automatically groups tools into "virtual tool" containers. The agent sees a smaller, navigable catalog instead of all schemas at once, loading full definitions only when it selects a tool category.

The practical guideline: **enable only the MCP servers relevant to your current work.** In VS Code, you can toggle individual servers and tools from the tools picker in the Chat view. Configuring seven MCP servers "just in case" means the agent opens every session with thousands of tokens of context it won't use.

### Tool results: you have no control over size

There's a second context problem that's harder to solve: tool *results* have no built-in size limit in MCP. A tool that fetches 500 Sentry events returns all 500. That entire payload enters the agent's context window as a tool result message.

The protocol has no native pagination for tool execution outputs — pagination in MCP applies only to the listing/discovery phase (tools/list, resources/list), not to what a tool returns when it runs.

Mitigations:

1. **Allowlist narrow tools.** A `get_issue_summary` tool that returns one paragraph is cheaper than a `get_issues_list` tool that returns 50 full issue objects. When configuring, favor tools designed for focused retrieval over bulk-export tools.

2. **Use resources instead of tools for large content.** Resources are opt-in. The agent (or you) attaches them explicitly when needed, rather than having them automatically flow into context on every tool call.

3. **Configure the tool allowlist tightly** (covered in the security section below). Fewer enabled tools = fewer opportunities for large payloads to enter context unexpectedly.

### A security angle you can't ignore: Tool Poisoning Attacks

Context management intersects with security in a critical way. In April 2025, Invariant Labs published research documenting the **Tool Poisoning Attack (TPA)**: malicious instructions embedded in MCP tool `description` fields.

Here's why this matters for context: **the model sees tool descriptions in full; the user typically sees only the tool name.** A description field might show "Adds two numbers" in your UI, while containing hidden text that instructs the model to read `~/.cursor/mcp.json` and exfiltrate credentials via a parameter.

Concrete documented example from the Invariant Labs paper:
...
The model follows the hidden instructions because they appear in its context. The user never sees them.

Related variants:
- **Shadowing attack**: a malicious server's tool description instructs the model to misuse tools from *other trusted servers* in the same context.
- **Rug pull attack**: a server changes its tool descriptions after initial approval. Most clients cache but don't re-verify descriptions per request.

VS Code's mitigations include sandboxed stdio servers and two-step URL approval (separate approval for request and response content). But the core defense is: **only install MCP servers from sources you trust and have reviewed**. A server from an unknown third party has full access to your agent's context.
...
5. **MCP has a context cost you must manage actively.** Tool schemas for all enabled tools are injected on every request. With many servers, this can reach 150,000 tokens of definitions alone. Enable only the servers relevant to the current task, use virtual tools in VS Code, and prefer narrow retrieval tools over bulk-export ones.

6. **Combining multiple MCP servers creates compounding value.** Sentry for error context plus Notion for design docs plus GitHub for project management gives the agent the same information sources you use — making its output dramatically more relevant.

In Ch 15, we'll build on MCP by adding **hooks and automation** — shell commands that execute at key points during agent execution, the `copilot-setup-steps.yml` file that customizes the agent's development environment, and workflows that take an issue all the way from creation to merged PR without manual intervention.
