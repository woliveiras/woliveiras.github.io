...
Module 4 is about earning, and keeping, their trust. In this chapter we'll cover the risks that come with AI-assisted development, the controls GitHub provides to mitigate them, and the governance structures that let organizations adopt AI coding agents confidently. In Ch 17, we'll shift from security to **impact measurement**, the metrics and feedback loops that prove your AI-assisted workflow is actually making things better.
...
The core rule is blunt: **AI output is untrusted input.** Generated code, generated shell commands, generated SQL, generated documentation, generated PR descriptions, MCP tool results, and issue comments read by an agent all need validation before they influence production systems.
...
For governance language, use the [NIST AI Risk Management Framework](https://www.nist.gov/itl/ai-risk-management-framework): govern, map, measure, and manage. That framing helps security, legal, and engineering discuss the same workflow without reducing it to "trust the model" or "ban the model."
...
2. **Use environment variables through setup steps.** In Ch 15, you learned about `copilot-setup-steps.yml`. Secrets referenced there use GitHub Actions secrets — they're injected at runtime, not stored in code:
...
2. **Hooks for pre-execution validation.** In Ch 15, you built hooks that intercept tool calls. A `preToolUse` hook on the `execute` tool can inspect the command before it runs:
...
5. **Treat external context as hostile until checked.** Issue bodies, comments, docs pages, API responses, and MCP tool descriptions can carry instructions. Tell agents explicitly to summarize external content as data, not to obey instructions embedded inside it.
...
5. **Require a dependency justification in the PR.** If the agent adds a package, the PR description should explain why standard library or existing dependencies were insufficient, which version was added, and which security/license checks passed.
...
In Ch 6, you learned to create custom agents, specialized profiles that focus on specific tasks. Those same profiles are also a security tool. By controlling which **tools** an agent can access, you control what it can do.
...
```yaml title=".github/agents/security-reviewer.agent.md"
---
...
```yaml title=".github/agents/implementation-agent.agent.md"
---
...
```yaml title=".github/agents/notion-researcher.agent.md"
---
...
5. **Monitor.** Use audit hooks (Ch 15) to log every agent action. Review logs weekly for unexpected behavior.

6. **Iterate.** Update the profile based on what the logs reveal. The cycle repeats.

Governance should not make safe work impossible. The rule of thumb I use: low-risk changes should move quickly through automated checks; high-risk changes should hit explicit human approval. Auth, payments, data deletion, migrations, production operations, and broad MCP write access belong in the second category.
...
```yaml title=".github/agents/do-everything.agent.md"
---
name: do-everything
